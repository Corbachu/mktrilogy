...
volatile int vid_dpipe_free;		// next dlist to be written to by game
...
int vid_get_dbuf( void ) {
	if( vid_dpipe_free >= 0 ) {
		...
		Mlp = dpipe[vid_dpipe_free].mbuf;
		Vlp = dpipe[vid_dpipe_free].vbuf;
		Plp = dpipe[vid_dpipe_free].pbuf;

		dpipe[vid_dpipe_free].zbuf = 0;
...
#endif
	mtx_rsp_fixup( dpipe[vid_dpipe_writing].mbuf, Mlp );
...
#if BENCHGFX
	bench_gfx( dpipe[vid_dpipe_reading].dbuf );
#endif
