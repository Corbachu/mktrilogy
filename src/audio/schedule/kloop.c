...
{
static int sid;
if( gamepad_click( 1, BUTTON_A )&BUTTON_A ) {
	printf(( "%d\n", sid ));
	wess_seq_trigger( sid++ );
}
if( gamepad_click( 1, BUTTON_B )&BUTTON_B ) {
	sid -= 2;
	printf(( "%d\n", sid ));
	wess_seq_trigger( sid++ );
}
if( gamepad_click( 1, BUTTON_C )&BUTTON_C ) {
	printf(( "%d\n", sid-1 ));
	wess_seq_trigger( sid-1 );
}
}
#endif
