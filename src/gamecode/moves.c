...
extern WORD scom_osz_fatal[];
extern WORD scom_robo_2zap1[];
...
extern WORD scom_lao_dbl_teleport[];
...
extern SMOVE sm_osm_hpc[];
extern SMOVE sm_osm_rc[];
...
extern SMOVE sm_osz_hkc[];
extern SMOVE sm_osz_bc[];
extern SMOVE sm_robo2_lkc[];

extern SMOVE sm_motaro_hpc[];
extern SMOVE sm_motaro_hkc[];
extern SMOVE sm_motaro_lko[];

extern SMOVE sm_sk_hpc[];
extern SMOVE sm_sk_hkc[];
extern SMOVE sm_sk_lkc[];
extern SMOVE sm_sk_rc[];

extern SMOVE sm_rain_hkc[];
extern SMOVE sm_rain_hkc[];
extern SMOVE sm_rain_hpc[];
extern SMOVE sm_rain_bc[];
extern SMOVE sm_rain_lpc[];

extern SMOVE sm_jc_hkc[];
extern SMOVE sm_jc_lkc[];
extern SMOVE sm_jc_hpc[];
extern SMOVE sm_jc_lpc[];

extern SMOVE sm_rd_rc[];
extern SMOVE sm_rd_uc[];
extern SMOVE sm_rd_lpo[];
extern SMOVE sm_rd_hpo[];
extern SMOVE sm_rd_hpc[];
extern SMOVE sm_rd_lko[];
extern SMOVE sm_rd_hkc[];

extern SMOVE sm_sa_bc[];
extern SMOVE sm_sa_hpo[];
extern SMOVE sm_sa_lpc[];
extern SMOVE sm_sa_hpc[];
extern SMOVE sm_sa_hkc[];
extern SMOVE sm_sa_rc[];

extern SMOVE sm_noob_rc[];
extern SMOVE sm_noob_bc[];
extern SMOVE sm_noob_hkc[];
extern SMOVE sm_noob_hpc[];
extern SMOVE sm_noob_lpc[];
extern SMOVE sm_noob_uc[];

void noob_run_close(void);
void noob_hk_close(void);
void rain_hk_close(void);
void rain_block_close(void);
void raiden_hk_close(void);
void raiden_hp_close(void);
void sa_hk_close(void);
void scorp_run_close(void);
void sa_run_close(void);
void osz_hk_close(void);
void osm_hp_close(void);
void rain_lp_close(void);
void jc_hk_close(void);
void jc_lk_close(void);
void jc_hp_close(void);
void jc_lp_close(void);
...
WORD stick_look_lr(WORD p1_legal,WORD p2_legal,WORD *scom)
{
	...
	if (illegal_button_check(obj,p1_legal,p2_legal)==SYSTEM_ZERO_SET || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
 ...
 * RAIN SPECIAL MOVES 		*
 ********************************/
void rain_hp_close(void)		
{
	secret_move_search(sm_rain_hpc);
}

void rain_block_close(void)		
{
	secret_move_search(sm_rain_bc);
}

void rain_lp_close(void)		
{
	secret_move_search(sm_rain_lpc);
}

void rain_hk_close(void)		
{
	secret_move_search(sm_rain_hkc);
}

/********************************
 * NOOB SPECIAL MOVES 		*
 ********************************/
void noob_hp_close(void)		
{
	secret_move_search(sm_noob_hpc);
}

void noob_run_close(void)		
{
	secret_move_search(sm_noob_rc);
}

void noob_block_close(void)		
{
	secret_move_search(sm_noob_bc);
}

void noob_hk_close(void)		
{
	secret_move_search(sm_noob_hkc);
}

void noob_up_close(void)		
{
	secret_move_search(sm_noob_uc);
}

void noob_lp_close(void)		
{
	secret_move_search(sm_noob_lpc);
}

/********************************
 * RAIDEN SPECIAL MOVES 		*
 ********************************/
void raiden_right_close(void)		
{
	secret_move_search(sm_rd_rc);
}

void raiden_hp_close(void)		
{
	secret_move_search(sm_rd_hpc);
}

void raiden_up_close(void)		
{
	secret_move_search(sm_rd_uc);
}

void raiden_lp_close(void)		
{
	secret_move_search(sm_rd_lpo);
}

void raiden_hp_open(void)		
{
	secret_move_search(sm_rd_hpo);
}

void raiden_hk_close(void)		
{
	secret_move_search(sm_rd_hkc);
}

void raiden_lk_open(void) 
{
	secret_move_search(sm_rd_lko);
}

/********************************
 * BARAKA SPECIAL MOVES 		*
 ********************************/
void sa_hp_close(void)		
{
	secret_move_search(sm_sa_hpc);
}

void sa_run_close(void)		
{
	secret_move_search(sm_sa_rc);
}

void baraka_hp_open(void)		
{
	secret_move_search(sm_sa_hpo);
}

void sa_hk_close(void)		
{
	secret_move_search(sm_sa_hkc);
}

void sa_lp_close(void)		
{
	secret_move_search(sm_sa_lpc);
}

void sa_block_close(void)		
{
#if 0
	WORD ta5;
	WORD *pfast;
	
	ta5=get_tsl(l_block,current_proc->pa8);
	
	update_tsl(l_block,current_proc->pa8);
	
	pfast=&c_p1p2_fastblk[(current_proc->pa8==p1_obj)?0:1];
	...
	if (get_tsl(l_spin,current_proc->pa8)<0x60) 
	{
		*pfast=0;
		process_suicide();
	}
	
	restricted_xfer(do_sa_spin);
#endif
	secret_move_search(sm_sa_bc);
...
{
	secret_move_search(sm_sk_rc);

}

void sk_hk_close(void) 
{
	secret_move_search(sm_sk_hkc);

}

void sk_lk_close(void) 
{
	secret_move_search(sm_sk_lkc);

}

void sk_hp_close(void) 
{
	secret_move_search(sm_sk_hpc);

}

/********************************
 * MOTARO SPECIAL MOVES 		*
 ********************************/
void motaro_hk_close(void) 
{
	secret_move_search(sm_motaro_hkc);

}

void motaro_hp_close(void) 
{
	secret_move_search(sm_motaro_hpc);

}

void motaro_lk_open(void) 
{
	secret_move_search(sm_motaro_lko);
...
void smoke_block_close(void)
{
	...
	if (get_his_action(current_proc)==ACT_REACT_FLIPK || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
		smoke_block_close2();
...
void lao_up(void)
{
	if (stick_look_lr(P1B0|P1B1|P1B2|P1B3|P1B4,P2B0|P2B1|P2B2|P2B3|P2B4,scom_lao_dbl_teleport)==SYSTEM_CARRY_SET)
		restricted_xfer(do_lao_dbl_tele);
...
{
	if (stick_look_lr(P1B3,P2B3,scom_lao_angle_kick)==SYSTEM_CARRY_SET && distance_from_ground(current_proc->pa8)>=SCY(0xa0) || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
...
{
	if (stick_look_lr(P1B0,P2B0,scom_osz_fatal)==SYSTEM_CARRY_SET && q_close_fatal()==SYSTEM_CARRY_SET) 
	{
		fatality_xfer(do_fatality_1);
	}
	else
	{
		...
		else
		{
			...
			else
			{
				...
				else secret_move_search(sm_osz_hpc);
//				else process_suicide();
			}
		}
	}
...
{
	if (stick_look_lr(P1B1,P2B1,scom_robo_2zap1)==SYSTEM_CARRY_SET)
	{
		if (process_exist(p1p2_pick(current_proc->pa8,PID_P1ROCKET2,PID_P2ROCKET2),-1)!=NULL)
			process_suicide();
		else restricted_xfer(do_robo_2zap);
	}
...
void robo_hp_close(void)
{
	...
	{
		pid=p1p2_pick(current_proc->pa8,PID_P1ROCKET1,PID_P2ROCKET1);
...
//		if (process_exist(p1p2_pick(current_proc->pa8,PID_P1ROCKET1,PID_P2ROCKET1),-1)!=NULL)
//			process_suicide();

		if (count>2)
			process_suicide();
		else restricted_xfer(do_robo_zap2);
	}
	else process_suicide();
}
...
{
	do_stationary(STAT_RAIN_BOLT);
}

void do_noob_alterego(void)
{
	do_stationary(STAT_NOOB_EGO);
}


void do_raiden_shocker(void) 
{
	do_stationary(STAT_RAIDEN_SHOCKER);
}


void do_shred(void) 
{
	do_stationary(STAT_SA_SHRED);
}


void do_sa_spin(void) 
{
	do_stationary(STAT_SA_SPIN);
}
...
void do_ind_charge_fast(void)
{
	do_body_propell(PROP_IND_CHARGE_FAST);
}
...
{
	do_body_propell(PROP_LAO_DBL_TELE);
}

void do_jc_shoruken(void)
{
	do_body_propell(PROP_JC_SHORUKEN);
}

void do_jc_shadowk(void)
{
	do_body_propell(PROP_JC_SHADOWK);
}

void do_jc_shadowkr(void)
{
	do_body_propell(PROP_JC_SHADOWKR);
}
...
{
	do_body_propell(PROP_SCORP_TELEF);
}
...
void do_kano_pyscho(void) 
{
	do_body_propell(PROP_KANO_PYSCHO);
}

void do_raiden_dive(void) 
{
	do_body_propell(PROP_RAIDEN_DIVE);
}

void do_combo_raiden_dive(void) 
{
	set_half_damage(current_proc->pdata.p_otherguy);	// chill out dudes hit
	do_body_propell(PROP_RAIDEN_DIVE);
}

void do_raiden_melt(void) 
{
	do_body_propell(PROP_RAIDEN_MELT);
}
...
void do_noob_slam(void)
{
	...
	do_body_propell(PROP_NOOB_SLAM);
}
...
void do_jc_zaphi(void)
{
	do_zap(ZAP_JC_SNOTH);
}

void do_jc_zaplo(void)
{
	do_zap(ZAP_JC_SNOTL);
}
...
void do_swat_bombshilo(void)
{
	do_zap(ZAP_SWAT_BOMBSHL);
}

void do_swat_bombslohi(void)
{
	do_zap(ZAP_SWAT_BOMBSLH);
}
...
void do_lia_forward2(void)
{
	do_zap(ZAP_LIA_2FZAP);
}
...
{
	do_zap(ZAP_THROW_BOLT);
}

void do_reverse_throw_lightning(void) 
{
	do_zap(ZAP_RD_REVERSE);
}
...
void fatality_xfer(FUNC fatal_func)
{
	...
	*/
	if (f_no_blood !=0 || winner_status !=3 || f_death!=0 || is_he_airborn()==SYSTEM_CARRY_SET)
	{
		process_suicide();
	}
...
{
	shang_morph(FT_RAYDEN);
}

void do_st_2_baraka(void)
{
	shang_morph(FT_BARAKA);
}
...
void shang_morph(WORD pa9)
{
	...
	if (pa9==FT_SMOKE || pa9==FT_OLDSMOKE)
		create_fx(FX_SMOKE);
...
{
	return(q_shang_morph(FT_SMOKE));
}

WORD q_shang_morph_osm(void)
{
	return(q_shang_morph(FT_OLDSMOKE));
}

WORD q_shang_morph_rain(void)
{
	return(q_shang_morph(FT_RAIN));
}
...
WORD q_shang_morph_kit(void)
{
	return(q_shang_morph(FT_KITANA));
}

WORD q_shang_morph_rep(void)
{
	return(q_shang_morph(FT_REPTILE));
}

WORD q_shang_morph_noob(void)
{
	return(q_shang_morph(FT_NOOB));
}

WORD q_shang_morph_ermac(void)
{
	return(q_shang_morph(FT_ERMAC));
}

WORD q_shang_morph_osz(void)
{
	return(q_shang_morph(FT_OLDSZ));
}

WORD q_shang_morph_mileena(void)
{
	return(q_shang_morph(FT_MILEENA));
}

WORD q_shang_morph_rayden(void)
{
	return(q_shang_morph(FT_RAYDEN));
}

WORD q_shang_morph_baraka(void)
{
	return(q_shang_morph(FT_BARAKA));
}

WORD q_shang_morph_jade(void)
{
	return(q_shang_morph(FT_JADE));
}

WORD q_shang_morph_scorpion(void)
{
	return(q_shang_morph(FT_SCORPION));
}
...
//		q_no;						// no morph during randper
	...
	if (p_agressor_active[(current_proc->pa8==p1_obj)?0:1]!=AGRESS_NORMAL)
		q_no;
	else q_yes;
}
...
WORD q_rain_zap(void)
{
	if (get_his_p_hit(current_proc)>=4 || get_his_action(current_proc)==ACT_REACT_STORM)
		q_no;
	else q_yes;
}
...
WORD q_hand_fatal(void)
{
	return(q_fatal_dist(SCX(0x90),SCX(0xc8)));
}
...
WORD q_noob_ego(void)
{
	PROCESS *ptemp;
	
	if (process_exist(p1p2_pick(current_proc->pa8,PID_P1_EGO,PID_P2_EGO),0xffff)!=NULL || get_tsl(l_decoy,current_proc->pa8)<(0x40*3+0x10))
		q_no;
	else q_yes;
}

WORD q_noob_zap(void) 
{
	if ((current_proc->pdata.p_otherproc)->pdata.p_hit>=4)
		q_no;
	q_yes;	
}
...
{
	if (get_his_p_hit(current_proc)>=1 || get_tsl(l_sz_decoy,current_proc->pa8)<(0x40*4) || get_his_action(current_proc)==ACT_STUNNED)
		q_no;
	else q_yes;
}

WORD q_sk_fatal_hammer(void) 
{
	return(q_fatal_dist(SCX(0x80),SCX(0xe0)));
}

WORD q_mot_fatal_head(void) 
{
...
WORD q_tusk_blur(void)
{
	...
	if (t_a1==ACT_FK_SD || t_a1==ACT_RET_KICK || t_a1==ACT_BLURRED || t_a1==ACT_SWEEP_SD || t_a1==ACT_PUNCHED)
		q_no;
...
WORD q_noob_slam(void) 
{
	if (get_tsl(l_warp,current_proc->pa8)<0x60)
		q_no;
	else q_yes;
}
...
WORD q_rd_melt(void) 
{
	if (get_tsl(l_warp,current_proc->pa8)<0x40)
		q_no;
	else q_yes;
}
...
{
	if (get_tsl(l_lk,current_proc->pa8)<0x80)
		q_no;
...
{
	if (get_tsl(l_hp,current_proc->pa8)<0x40*4)
		q_no;
	return(q_close_fatal());	
}

WORD q_rd_shocker(void) 
{
	if (get_tsl(l_hp,current_proc->pa8)<0x38 || is_he_joy()==SYSTEM_CARRY_CLR || get_his_action(current_proc)==ACT_SHOCK_SD)
		q_no;
	else q_yes;
}
...
{
	if (get_his_p_hit(current_proc)>=2 || process_exist(p1p2_pick(current_proc->pa8,PID_DECOY1,PID_DECOY2),-1)!=NULL || process_exist(p1p2_pick(current_proc->pa8,PID_FLOOR_ICE1,PID_FLOOR_ICE2),-1)!=NULL || get_his_action(current_proc)==ACT_PUNCHED)
		q_no;
	else q_yes;
}
...
WORD q_osz_spike_fatal(void)
{
	get_his_dfe();
	if (current_proc->a5<SCX(0x80) || current_proc->a6<SCX(0x80))
		q_no;
	else return(q_close_fatal());
}
...
{
	if ((curback==BKGD_HADES_MOD ||curback==BKGD_PIT_MOD || curback==BKGD_BELL_MOD || curback==BKGD_SUBWAY_MOD || curback==BKGD_TOMB_MOD || curback==BKGD_DEADPOOL_MOD) && f_no_blood==0)
	{
...
WORD q_noob_animal(void)
{
	return(q_animal_dist(SCX(0xb0),SCX(0xc0)));
}
...
{
	if (is_master_in_finish()==SYSTEM_CARRY_CLR || f_mercy!=0 || get_x_dist<SCX(0xa0) || f_special_match==1)
		q_no;
...
{
	if (get_his_action(current_proc)==ACT_LAO_SPINNED || get_his_p_hit(current_proc)>2)
		q_no;
	else q_yes;
}
...
WORD q_baraka_fatal(void) 
{
	return(q_fatal_dist(SCX(0x00),SCX(0x80)));
}

WORD q_baraka_decap(void) 
{
	return(q_fatal_dist(SCX(0x00),SCX(0x60)));
}
...
{
	if (get_tsl(l_block_fk,current_proc->pa8)<0x20 || get_tsl(l_lk,current_proc->pa8)<0x28)
		q_no;
	else q_yes;
}
...
WORD q_rain_bolt(void) 
{
	if ((current_proc->pdata.p_otherguy)->ochar==FT_SK || (current_proc->pdata.p_otherguy)->ochar==FT_MOTARO ||get_his_p_hit(current_proc)>=3 || get_tsl(l_zap,current_proc->pa8)<0x70)
		q_no;
	else q_yes;	
}
...
extern SMOVE sm_home_codes1[];
void home_code_que(WORD swoffset,LONG *b_queue)
{
	switch_queue(swoffset,b_queue);

	home_code_check(0,sm_home_codes);		// check player 1 for codes set 0
	home_code_check(1,sm_home_codes);		// check player 2 for codes	set 0
#if 0	
	home_code_check(0,sm_home_codes1);		// check player 1 for codes	set 1
	home_code_check(1,sm_home_codes1);		// check player 2 for codes	set 1
#endif
	process_suicide();
}
...
void hc_ukk(void) 
{
	...
	if ((f_oldsm==0 || f_kamel==0) && gstate!=GS_UNLOCK_HID)
	{
		ptemp=process_exist(PID_AMODE,0xffff);
		if ( ptemp!=NULL )
			fastxfer(ptemp,ukk_codes);
	}
	return;
}
