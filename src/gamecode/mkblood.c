...
#define MAX_BLOOD_PROCS	38

WORD count_blood_procs(void) 
{
	PROCESS *ptemp;
	WORD count=0;
	...
	{
		if (ptemp->procid==PID_BLOOD || ptemp->procid==PID_GRAVP)
			count++;
			
		ptemp=ptemp->plink;
	}
	
	if (count>MAX_BLOOD_PROCS)
		q_no;
	else q_yes;
}
...
void stabbed_blood(void) 
{
	current_proc->a11=current_proc->a10;
...
void stabbed_spirt(void) 
{
	current_proc->a10=0x14;
	do
	{
		CREATE(PID_BLOOD,stabbed_stream);
		process_sleep(2);
...
void stabbed_stream(void) 
{
	get_jt_blood;
	a11_blood_lineup(SCX(0x0),SCY(0x38));
	(current_proc->pa8)->oyvel.pos=-(randu(SCY(0x30000))+SCY(0x30000));
	
	set_proj_vel(current_proc->pa8,SCX(0x20000)+randu(SCX(0x20000)),-1);

	insert_object(current_proc->pa8,&objlst);

	current_proc->pa9=a_drip;
	current_proc->a0=6;
	current_proc->a10=SCY(0x6000);
	drip_fall();
}

void blood_shred(void) 
{
	short ta6;
	OBJECT *obj;
	
	obj=((PROCESS*)current_proc->a10)->pdata.p_otherguy;
	...
	do
	{
		...
		do
		{
			current_proc->pa8=obj;		// set so color blood will appear
			current_proc->a11=(ADDRESS)current_proc->pa8;
			if (count_blood_procs()==SYSTEM_CARRY_SET)
				CREATE(PID_BLOOD,shred_drip);
		}
		while(--ta6>0);
		process_sleep(5);
	}
	while(((PROCESS*)current_proc->a10)->pdata.p_action==ACT_SHRED);
	
	process_suicide();
}

void shred_drip(void) 
{
	OBJECT *ta11=(OBJECT*)current_proc->a11;
	OBJECT *obj;
	
	get_jt_blood;
	obj=current_proc->pa8;
	
	obj->oypos.u.intpos=SCY(0x30)+randu(SCY(0x30))+ta11->oypos.u.intpos;
	
	obj->oxpos.u.intpos=ta11->oxpos.u.intpos+((ta11->oflags & M_FLIPH) ? -SCX(0x30):SCX(0x30));
	
	/* set vel */
	obj->oxvel.pos=randu(SCX(0x40000))+SCX(0x4000);
	if (randper(300)==SYSTEM_CARRY_SET)
		obj->oxvel.pos*=-1;
	
	insert_object(obj,&objlst);
	
	obj->oyvel.pos=-(randu(SCY(0x40000))+SCY(0x30000));
...
void sawb3(void)
{
	...
	current_proc->a4=0x00810001-(adjust<<16);
	current_proc->a5=0x00040000;
	spawn_drip_a10(2);

	current_proc->a10=adjust;
	if (count_blood_procs()==SYSTEM_CARRY_SET)
		CREATE(PID_BLOOD,saw_right);
		
	get_tg_blood;
	lineup_with_a11_offset(SCX(0x0005),SCY(0x0088)-adjust);
	flip_single(current_proc->pa8);
	stack_jump(upcb3);
}
...
void saw_right(void)
{
	...
	get_tg_blood;
	lineup_with_a11_offset(SCX(0x0000),SCY(0x0088)-adjust);
	stack_jump(upcb3);
}
...
void spike_blood(void)
{
	current_proc->a10=0x2b;
	current_proc->a11=4;
	do
	{
		if (count_blood_procs()==SYSTEM_CARRY_SET)
...
void gbld3(void *pa5)
{
	...
	gso_dmawnz(obj,(ADDRESS)pa5,blood_dict,0,0);
	alloc_cache(pa5,0,obj);
	
	obj->oflags|=M_XLU_ON;
	obj->oflags|=M_XLU_BLOOD;
...
void spike_drips(void) 
{
	...
	ta10=spike_drip_spots;	
	while (*ta10!=0)
	{
		ptemp=CREATE(PID_BLOOD,one_spike_drop);
		if (ptemp!=NULL)
			ptemp->a10=(long)(*ta10);
		ta10++;
		process_sleep(10);
...
void one_spike_drop(void) 
{
	OBJECT *obj;
	OBJECT *ta3=current_proc->pa8;				// save player object we are following
	
	current_proc->a11=(ADDRESS)current_proc->pa8;
	
	gso_dmawnz(obj,(ADDRESS)STAB1,blood_dict,0,0);
	alloc_cache((OIMGTBL *)STAB1,0,obj);
	...
	obj->oypos.u.intpos=((OBJECT *)current_proc->a11)->oypos.u.intpos+SCY(0x88);
	obj->oxpos.u.intpos=((OBJECT *)current_proc->a11)->oxpos.u.intpos+(long)current_proc->a10;
	
	insert_object(obj,&objlst);
	current_proc->pa9=a_rotate_7;
	framew(4);
	
	current_proc->pdata.p_ganiy=ground_y+randu(7);
	
	current_proc->a0=8;
	current_proc->pa9=a_drip;
	find_last_frame();
	(long)current_proc->a10=SCY(0x3000);
