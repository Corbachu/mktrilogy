...
PROCESS *process_create(WORD procid,void (*func)(void))
{
	...
	if (free_head != NULL)
	{
		...
		new_proc->p_context.c_frameptr=new_proc->p_context.c_stackptr=(ADDRESS)&(new_proc->p_context.c_stack[STKSIZE/8-3]);	/* set start of stack for process
	...
	if (procid==PID_BLOOD || procid==PID_GRAVP || procid==PID_ROCK)
		return(NULL);
...
int process_free(void)
{
	PROCESS *proc_ptr;
	int i=0;

	proc_ptr=(PROCESS *)&free_head;
