...
void create_invaders(void);
void invader_enemy(void);
void move_iwave(void);
...
void iwatch_buttons_or_missiles(WORD pa0);
...
extern BYTE arcade_dict[];
void create_player_2_ship(void)
{
	OBJECT *obj;

	gso_dmawnz(obj,(ADDRESS)&ship3,arcade_dict,0,0);
	alloc_cache(&ship3,0,obj);
...
void create_player_1_ship(void)
{
	...
	gso_dmawnz(obj,(ADDRESS)&ship1,arcade_dict,0,0);
	alloc_cache(&ship1,0,obj);
	set_xy_coordinates(obj,(game_type==0)?SCX(0x50):160,(game_type==0)?SCY(0xe0):SCY(0xe0)+16);
	player_1_ship=obj;
	insert_object(obj,&objlst);
	return;
}
...
void start_enemy_missile(void)
{
	...
	make_solid_object((game_type==0)?0xe001:0xe011,3,6);
...
void restart_player_1(void)
{
	process_sleep((game_type==0)?100:50);
	create_player_1_ship();
	process_suicide();
}
...
void create_baiters(void)
{
	...
	do
	{
		...
		gso_dmawnz_ns(obj,(ADDRESS)pa10->b_frame,arcade_dict,0,0);
		alloc_cache(pa10->b_frame,0,obj);
...
void be_9(WORD flag)
{
	if (flag==2) 
	{
		wave_x_velocity+=0x1000;	// make them move faster
	}
...
void ce4b(void)
{
	...
	gso_dmawnz(obj,(ADDRESS)&expld1,arcade_dict,0,0);
	alloc_cache(&expld1,0,obj);
...
void ce4s(void)
{
	...
	gso_dmawnz(obj,(ADDRESS)&expld6,arcade_dict,0,0);
	alloc_cache(&expld1,0,obj);
	temp.yx=(LONG)current_proc->pa9;
...
OBJECT *makestar(void)
{
	...
	obj=make_solid_object(0x7bdf,2,2);	
	obj->ozval=0;
	obj->oxpos.u.intpos=randu(SCX(400))-1;
...
//********************************************************************************************
//		CLUE SYSTEM SPACE INVADERS 
//********************************************************************************************
extern char *secret_clue_table[];
char clue_decode[255];

void clue_decode_routine(BYTE *cptr) 
{
	int i,j;
	char code;
	char *dptr=clue_decode;
...
void invade_clue_system(WORD numclues,WORD maxclue) 
{
	...
	if (maxclue>16)	 
	{
		if (df_nopower==1 || cmos_diff==0 || cmos_diff==1) 
		{
			maxclue=16;
			if (numclues>3)
				numclues-=2;
				
			current_proc->a0=(ADDRESS)cheat_message;
			current_proc->pdata.p_store1=0x1a;
			winner_msg_rise();
			f_novel=0;
	...
	/* pick correct message and have it rise */
	for (i=0;i<numclues;i++)
	{
		clue_decode_routine((BYTE *)secret_clue_table[clues[i]]);
		current_proc->a0=(ADDRESS)clue_decode;
		current_proc->pdata.p_store1=0x1a;
		winner_msg_rise();
		f_novel=0;
...
//		SPACE INVADERS 
//********************************************************************************************
WORD space_invade(WORD pa0)		// pa0=player to control stuff
{
	WORD ta10,ta11;
...
//	CREATE(PID_PONG_SPEED,make_stars);
	...
	player_2_ships=3;
	create_player_1_ship();
	wave_x_velocity=0x2000;
	
	shoot_randper=dive_randper=3;
	
	/* create enemys */
	create_invaders();
	CREATE(PID_PONG_SPEED,move_iwave);
	...
	do
	{
		...
		do
		{
			/* move player */
			{
				...
				ta0=(pa0==0)?read_player_1_joystick():read_player_2_joystick();
				ta0<<=1;
				if ( player_1_ship!=NULL )
					mpss1(ta0,player_1_ship);
			}
			
			iwatch_buttons_or_missiles(pa0);
...
HGSH9:
	...
	f_nosound=0;
	return((player_1_ships>0)?SYSTEM_CARRY_SET:SYSTEM_CARRY_CLR);
}
...
void create_invaders(void)
{
	...
	do
	{
		...
		set_xy_coordinates(obj,pa10->b_xpos+wave_x_position,pa10->b_ypos+wave_y_position);
		insert_object(obj,&objlst);
		CREATE(PID_PONG_SPEED,invader_enemy);
...
void invader_enemy(void)
{
	...
	do
	{
		...
		if (obj->oxpos.u.intpos<0x10 && wave_dir==-1)
			wave_change=1;
		
		if (obj->oxpos.u.intpos>288 && wave_dir==1) 
		{
			wave_change=-1;
		}
		...
		{
			if (randper(shoot_randper)==SYSTEM_CARRY_SET)
			{
				start_enemy_missile();
				//start_enemy_missile();
			}
			current_proc->pdata.p_store5=20;
		}
...
void move_iwave(void)
{
	...
	do
	{
		...
		if (wave_change!=0) 
		{
			wave_dir=wave_change;						// change direction
			wave_y_position+=12;		// move everything down a row
			wave_x_pos.pos-=wave_x_velocity;
		...
		if (wave_dir==1)
			wave_x_pos.pos+=wave_x_velocity;
		else wave_x_pos.pos-=wave_x_velocity;
	}
	while(1);
}
...
void iwatch_buttons_or_missiles(WORD pa0)
{
	...
	if ( player_1_ship!=NULL )
	{
		ta0=(pa0==0)?read_player_1_button():read_player_2_button();
		if ( ta0==0 )
		{
			player_1_bstate=ta0;
		}
