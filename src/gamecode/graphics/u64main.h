...
#define GLIST_LEN	(32*1024)

typedef struct dyn
{ 
  Gfx glist[GLIST_LEN];			// graphics list
  BYTE *idcmp_base;				// base address of decomp buffer for this dynamic structure
  WORD *draw_ptr;				// base address of frame buffer for this dynamic structure to render
  WORD *disp_ptr;				// base address of frame buffer for this dynamic structure to display
...
/* texture pool area */
#define TEXTURE_POOL_SIZE	(305*1024)
...
#define DECOMP_BUFFER_SIZE	(1024*116)			/* # of bytes in image decomp buffer */
