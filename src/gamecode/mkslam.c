...
void robo2_air_slam(void)
{
	...
	kano_double_mframew(3);					// step #2 throw
	if ((current_proc->pdata.p_otherproc)->pdata.p_flags & PM_CORPSE)		
		goto SLAM_ABORT;
...
/******************************************************************************
 Function: WORD read_lp_tick_state(OBJECT *obj)
...
void raiden_slam(void)
{
	...
	xfer_to_thrown(thrown_by_raiden);
...
void thrown_by_raiden(void) 
{
	OBJECT *obj=current_proc->pa8;
	
	obj->oflags &=~M_FLIPV;
	damage_to_me(20,obj);
	find_ani_part2(ANIM_FB_RAYDEN);
	do_next_a9_frame(obj);
	...
	flight(obj,SCX(0x40000),-SCY(0x60000),SCY(0x8000),0xfff,-1);
...
void thrown_by_baraka(void) 
{
	...
	flight(obj,SCX(0xb0000),-SCY(0x20000),SCY(0x8000),6,-1);
	damage_to_me(20,obj);
...
void baraka_slam(void)
{
	...
	xfer_to_thrown(thrown_by_baraka);
...
void body_slam_init(OBJECT *obj)
{
	...
	/* update agressor */
	player=(current_proc->pa8==p1_obj)?0:1;
	if (p_agressor_active[player]==AGRESS_NORMAL) 
		agressor_increaser(AGRESSOR_HIT,player,current_proc->pa8);
