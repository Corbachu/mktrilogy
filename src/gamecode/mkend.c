...
void scroll_paragraph(BYTE *ta11) 
{
	...
	do
	{
		pds_centered_soff(ta11,160,SCY(0xff));
		set_letter_vel();
...
SCP4:
		...
		if (ta0==-2) 
		{
			...
			while((current_proc->pa8)->oypos.u.intpos>SCY(0x14)+10);
...
WORD decode_code(WORD *ta5,OBJECT *oa8);
void release_entry(void);
void count_down_timer(void);
void count_shape(OBJECT *obj);
void trick_buttons(int curr,int prev);
void ukk_check_button(OBJECT *obj);
OBJECT *backdrop_box(WORD pa7);
void scan_1_ukk_set(OBJECT *oa8);

char txt_wrong[]="- INCORRECT CODE -";
char txt_from_this[]="FROM THIS POINT ON....";
char txt_ukk[]="ENTER THE\n ULTIMATE KOMBAT KODE\n";
char txt_smoke_is_act[]="SMOKE AND KHAMELEON\nARE AT YOUR CONTROL";
char txt_ukk_gameover[]="- GAME OVER -";
WORD the_code[]={1,1,3,8,4,0,0xffff};
...
void ukk_codes(void)
{
	...
	if (f_oldsm==1 && f_kamel==1)
	{
		ultimate_exit();
		return;
	}

	gstate=GS_UNLOCK_HID;
	...
	p15_centered(txt_ukk,SCX(0xc0),SCY(0x34)+16);
	CREATE(PID_HID_TIMER,count_down_timer);
	p15_centered(txt_ukk_gameover,SCX(0xc0),SCY(0x10)+16);

	oa9=backdrop_box(SCY(0x6d)+24);
	...
	do
	{
		...
		pmask=cmask;
		cmask=swcurr.padvalue;
		trick_buttons(cmask,pmask);
		
		scan_1_ukk_set(oa9);
		
	} while(process_exist(PID_HID_TIMER,0xffff)!=NULL);
		
	/* decode */
	if (decode_code(the_code,oa9)==SYSTEM_CARRY_SET)
		stack_jump(release_entry);
...
void count_down_timer(void)
{
	...
	objnum->oypos.u.intpos=SCY(0xa0)+32;
	center_obj_x(objnum);
...
int mask_table[]=
{
	0x00080008,				// BUTTON 0 (L1)
	0x00100010,				// BUTTON 1 (R1)
	0x00200020,				// BUTTON 2 (SQUARE)
	0x00040004,				// BUTTON 3 (TRIANGLE)
	0x00800080,				// BUTTON 4 (CIRCLE)
...
void ukk_check_button(OBJECT *obj)
{
	...
	if (*ta0 != 0 )
	{
		...
		offset=obj->oconst+(long)current_proc->a3;
		if (offset<0)
			offset=9;
		if (offset>9)
			offset=0;
...
OBJECT *backdrop_box(WORD pa7) 
{
	...
	oa8=obj=make_solid_object(0x001f,116,SCY(0x15)+1);
	obj->oypos.u.intpos=pa7;
	center_obj_x(obj);
	insert_object(obj,&objlst);
	current_proc->a10=(ADDRESS)obj;
	current_proc->pa9=(void *)obj;
	current_proc->a11=obj->oxpos.u.intpos;
	current_proc->a11+=2;

	current_proc->pdata.p_anicount=6;
	/* smoke 3 */
	do
	{
		animframe=(ADDRESS)COMPUTE_ADDR(0,symbol_table[0]);
		gso_dmawnz_ns(obj,animframe,symbol_dict,0,0);
		alloc_cache((OIMGTBL *)animframe,0,obj);
		insert_object(obj,&objlst);
		obj->oypos.u.intpos=((OBJECT *)current_proc->a10)->oypos.u.intpos+2;
		obj->oxpos.u.intpos=current_proc->a11;
		((OBJECT*)current_proc->pa9)->oslink=(void *)obj;
		(OBJECT*)current_proc->pa9=obj;
		obj->oslink=NULL;
		
		current_proc->a11+=(2+obj->osize.u.xpos);
		obj->oconst=0;
	}
