...
void arrow_red_check(void) 
{
	if (q_his_react_flag_set()==SYSTEM_CARRY_SET)
		swpal(REDFLM_P,current_proc->pdata.p_slave);
}

extern WORD RFIRE_p[];
void axe_red_check(void) 
{
	if (current_proc->pdata.p_store1==1)
		swpal(RFIRE_p,current_proc->pdata.p_slave);
}


void adj_rain_proj(void) 
{
	adjust_xy_a5((OBJECT *)current_proc->a10,0,-9);
}

void adj_noob_proj(void) 
{
	adjust_xy_a5((OBJECT *)current_proc->a10,0,-0x9);
}

void low_z_slave_pri(void) 
{
	(current_proc->pdata.p_slave)->ozval=1;
}
void slave_palette_change(void) 
{
	WORD pal;
	
	pal=(current_proc->pa8)->opal;
	copy_pal(current_proc->pdata.p_slave,pal);
	return;
}
...
void animate2(OBJECT *obj,ADDRESS *framelist)
{
	...
	/* alloc image in cache */
	obj->header.t_width=((OIMGTBL *)pram_ptr)->size.u.xpos; 
	obj->header.t_height=((OIMGTBL *)pram_ptr)->size.u.ypos; 
	obj->ovcache=alloc_vram_mem(obj->odict,(void *)pram_ptr->prsagptr,obj->ovcache,VCACHE_COMPRESS);
	...
	while (animframe != ANI_END)
	{
		switch (animframe)
		{
			...
			case ANI_SLADD:			// animation script add slave
				ptemp=gmo_proc((current_proc->pa8)->odict,(current_proc->pa8)->oheap);				// get process which holds data
