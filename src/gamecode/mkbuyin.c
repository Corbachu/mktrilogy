...
void do_buyin(void)
{
	...
	if (p1_state==PS_BUYIN)			// p1 in buyin --> he gets text
	{
		...
		p15_centered(cptr,84,163+8-3+16);
		move_menu_text_down();
	}
	...
	if (p2_state==PS_BUYIN)			// p2 in buyin -- he gets text
	{
		...
		p15_centered(cptr,240,163+8-3+16);
	...
	*/
	...
	l_hp[0]=SLIDE_TIME+4;				// set ticks till we stop menu
...
void shape_buyin_num(WORD num,OBJECT *obj_num)
{
	...
	fptr=*(font_lists[FONT_15]+(WORD)'0'-0x21+num);

	/* load correct number */
	obj_num->osag=fptr;
	obj_num->oimg=(LONG)fptr;	//-u64-
	obj_num->oheap=0;			//-u64-
	
	ta8=current_proc->pa8;
	current_proc->pa8=obj_num;
	animate(SINGLE_ANI0,fptr,obj_num->oflags);
