...
#define SYSTEM_CARRY_CLR        0x4000 /* carry set bit off */
				/* create mask for carry bits */
#define SYSTEM_CARRY_MASK       (SYSTEM_CARRY_SET+SYSTEM_CARRY_CLR) 

#define SYSTEM_ZERO_SET 	0x0002 /* zero set bit on */
#define SYSTEM_ZERO_CLR 	0x0001 /* zero set bit off */
				/* create mask for zero bits */
#define SYSTEM_ZERO_MASK        (SYSTEM_ZERO_SET+SYSTEM_ZERO_CLR) 
...
#define abs(_x) ((((int32_t)_x)<0)?-(_x):(_x))
/*
 *      Extract the low and high from a long
 */
#define LOWORD(theValue)        ((uint16_t)(theValue))
#define HIWORD(theValue)        ((uint16_t)((uint32_t)(theValue)>>16))
...
typedef uint16_t (*FUNCW)();                      /* void function with a word return */
...
#define SCX(_x) ((int32_t)(SCALE_FACTOR_X*(int32_t)(_x)))
#define SCY(_y) ((int32_t)(SCALE_FACTOR_Y*(int32_t)(_y)))

#define SCX_V(_x,_v) ((int32_t)((_v)*(int32_t)(_x)))
#define SCY_V(_y,_v) ((int32_t)((_v)*(int32_t)(_y)))
