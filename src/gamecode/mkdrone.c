...
void run_in_close_now(void)
{
	...
	if (q_is_he_airborn()==SYSTEM_CARRY_SET)	// rev1.2
		d_fflip_jump();
...
short indian_prop[]={2,PROP_IND_CHARGE,PROP_IND_CHARGE_FAST};
short kano_prop[]={2,PROP_KANO_CANNON,PROP_KANO_PYSCHO};
short lk_prop[]={2,PROP_LK_BIKE,PROP_LK_SBIKE};
short scorp_prop[]={2,PROP_SCORP_TELE,PROP_SCORP_TELEF};
short jcage_prop[]={3,PROP_JC_SHADOWK,PROP_JC_SHORUKEN,PROP_JC_SHADOWKR};

short *ochar_xtra_props[]= 
{
	indian_prop,
	kano_prop,
	lk_prop,
...
{
	if (diff<3 || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
		diff_no_propell();
...
void d_propell_attack_now(void)
{
	...
	ta0=ochar_props[(current_proc->pa8)->ochar];
	...
	else 
	{
		if (ta0 & 0x8000)
		{
			pa0=ochar_xtra_props[ta0 & 0x7fff];
			ta1=*pa0;
			
			if (diff<4) 		
				ta1/=2;						// easy difficuties only use half of arensal
			
			ta0=*(pa0+randu(ta1));			// pick one of our great moves
		}
...
short jax_zap[]={2,ZAP_JAX_ZAP1,ZAP_JAX_ZAP2};
short swat_zap[]={4,ZAP_SWAT_BOMBH,ZAP_SWAT_BOMBL,ZAP_SWAT_BOMBSHL,ZAP_SWAT_BOMBSLH};
short robo1_zap[]={2,ZAP_ROBO_ZAP,ZAP_ROBO_2ZAP};
short lia_zap[]={2,ZAP_LIA_FZAP,ZAP_LIA_2FZAP};

short *ochar_xtra_zaps[]= 
{
	jax_zap,
	swat_zap,
	robo1_zap,
...
void d_zap(void)
{
	...
	if (is_throwing_allowed()==SYSTEM_CARRY_CLR || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
		diff_no_zap();
...
void d_zap_now(void)
{
	...
	ta0=ochar_zaps[((current_proc->pa8)->ochar)];
	...
	else 
	{
		if (ta0 & 0x8000)
		{
			pa0=ochar_xtra_zaps[ta0 & 0x7fff];
			ta1=*pa0;
...
void d_slam(void)
{
	if (q_is_he_a_boss()==SYSTEM_CARRY_SET || q_is_he_airborn()==SYSTEM_CARRY_SET || q_am_i_noob_zap()==SYSTEM_CARRY_SET)
	{
...
void crossover_scan(void)
{
	...
	current_proc->a1=pa8->oxpos.u.intpos;
	current_proc->a2=(current_proc->pdata.p_otherguy)->oxpos.u.intpos;
...
{
	current_proc->a0=(current_proc->pdata.p_otherguy)->oypos.u.intpos;
	if ((short)current_proc->a0<0)
		q_no;

	current_proc->a1=(current_proc->pdata.p_otherproc)->pdata.p_ganiy;
...
void d_finish_him(void)
{
	...
	if (f_no_blood==0 && (current_proc->pdata.p_flags & PM_KAMEL)==0)
	{
		...
		if (randper(400)==SYSTEM_CARRY_SET)
		{
			...
			if (curback==BKGD_DEADPOOL_MOD || curback==BKGD_TOMB_MOD)
			{
				stack_jump(d_background_fatal);
			}
...
void drone_brutality(void)
{
	fatality_align(SCX(0x50));
	if (stance_wait_yes(0x40,q_is_he_dizzy)==SYSTEM_CARRY_CLR)
		d_fatality_abort();
	else
	{
		stack_jump(do_brutality);
	}
}
...
short ochar_fatality_distances[][2]=
{
	...
	{SCX(0x80),SCX(0x80)},		// scorp
	{SCX(0xe0),SCX(0x60)},		// rep
	{SCX(0xa0),SCX(0x48)},		// ermac
	{SCX(0x80),SCX(0x40)},		// oldsz
	{SCX(0xa0),SCX(0xa0)},		// oldsm
	{SCX(0xb0),SCX(0x90)},		// noob
	
	{SCX(0x40),SCX(0x40)},		// ray
	{SCX(0x40),SCX(0x40)},		// bara
	{SCX(0x48),SCX(0xb0)},		// rain
	{SCX(0x80),SCX(0x80)},		// kamel
};

void drone_execute_fatality(void)
{
	if (randper(180)==SYSTEM_CARRY_SET)		// determine if we want to have fun (small chance)
	{
	...
	/* def2 */
	if (randper(75)==SYSTEM_CARRY_SET)
	{
		drone_animality();
	}
...
JUMPTBL counter_attack_table[]=
{
	...
	c_sa_spin,		// 120 -u64-
	c_sa_shred,		// 121 -u64-
	NULL,	// 122 -u64- (cant shock drones)
	c_rain_bolt,	// 123 
};

void c_noob_tele(void);
...
JUMPTBL counter_sitting_duck[]=
{
	...
	c_slam_bounce,  	// 62e
	NULL,		// 62f
	c_sk_aircharge_sd,		// 630
	c_sk_charge_sd,		// 631
	c_rd_dive_sd,		// 632
	NULL,				// 633 (cant shock drones)
	NULL,				// 634 	//-DHS-
	c_rain_bolt_sd,				// 635 
	NULL,				// 636
	c_shoruken_sd,		// 637
...
JUMPTBL **ochar_cat_tables[]=
{
	...
	drone_catagory_table,		// 15
	drone_catagory_table,		// 16
	...
	drone_catagory_table,		// 24-u64-
	drone_catagory_table,		// 25
	drone_catagory_table,		// 26
	
	drone_catagory_table,		// 27
	drone_catagory_table,		// 28
	drone_catagory_table,		// 29
...
JUMPTBL crf_tbl[]= 
{
	...
	d_propell_attack_now,	//24
	d_do_shred,					//25
	drone_sweep_closeup_sd,	// 26
...
WORD rpt_bomb_sd[]={100,250,300,400,500,600,600,700,700,700};

void zap_close_away_flipper(void)
{
	dont_zap_teles();			// rev1.2
	...
	{
		if (ask_mr_diff(rpt_bomb_sd)==SYSTEM_CARRY_SET)
			d_zap_jump(ZAP_SWAT_BOMBSHL);
		else d_zap_jump(ZAP_SWAT_BOMBH);
	}
...
void dont_zap_teles(void) 		// rev1.2
{
	WORD ochar;
	
	ochar=(current_proc->pdata.p_otherguy)->ochar;
	if (ochar==FT_SCORPION || ochar==FT_ERMAC || ochar==FT_MILEENA || ochar==FT_NOOB)
		stack_jump(run_in_close);
	
	return;	
}

void d_shoruken(void) 
{
	 d_body_propell(PROP_JC_SHORUKEN);	
}
...
void st2rayden(void)
{
	if ( q_shang_morph(FT_RAYDEN)==SYSTEM_CARRY_SET )
	{
		do_st_2_rayden();
 		d_propell_attack_now();
	}
...
void c_sa_shred(void)
{
	react_jump_table_act(cav_shred);
	return;
}
...
void c_shadk(void)
{
	react_jump_table_act(cav_shadk);
	return;
}

void ct_shred(void) 
{
	if (d_get_x_dist<=can_drone_slam())
		d_slam();
...
void c_shoruken(void) 
{
	react_jump_table_act(cav_shor);
	return;	
}

WORD shoruken_wait(void) 
{
	if (distance_off_ground(current_proc->pdata.p_otherproc,current_proc->pdata.p_otherguy)>SCY(0x50))
		q_no;
	else q_yes;
}

void c_shoruken_sd(void) 
{
	if (rtb_nail_sd()==SYSTEM_CARRY_CLR)
	{
		return_to_beware;
	}

	if (d_get_x_dist>SCRRGT/2)
		d_zap();
	
	stance_wait_yes(0x40,shoruken_wait);
...
WORD slam_distances[]={SCX(0x46),SCX(0x46),SCX(0x46),SCX(0x46),SCX(0x46),SCX(0x48),SCX(0x4a),SCX(0x4d),SCX(0x50),SCX(0x50),SCX(0x50)};

WORD can_drone_slam(void) 
{
	return(slam_distances[diff]);
}
...
void c_sa_swipe(void) 
{
	react_jump_table_act(cav_swipe);
	return;
}
...
void c_sa_spin(void) 
{
	react_jump_table_act(cav_spin);
	return;
}

void ct_sa_spin(void) 
{
	if (d_get_x_dist>SCX(0x80))
		d_zap();
...
void ct_dive(void) 
{
	...
	ta1=get_char_stk(current_proc->pdata.p_otherguy,(current_proc->pdata.p_otherproc)->pdata.p_stk);
	ta3=(current_proc->pdata.p_otherguy)->oypos.u.intpos+ta1->syoff+ta1->sysize;
	
	if (ta3<highest_mpart(current_proc->pa8)) 
	{
		return_to_beware;
	}
...
WORD rpt_nail_sd[]={100,250,300,400,500,600,600,700,700,700};
WORD rtb_nail_sd(void) 
{
	return(ask_mr_diff(rpt_nail_sd));
}
...
void c_rd_dive_sd(void) 
{
	...
	current_proc->pdata.p_store1=randu(0x30)+0x40;
	stance_wait_yes(0x40,divesd_wait);
	d_zap();
}

WORD divesd_wait(void) 
{
	if (d_get_x_dist<current_proc->pdata.p_store1)
		q_no;
	else q_yes;
}
...
BYTE crd_check[]={5,5,4,4,3,3,2,2,1,1};
void c_reptile_dash(void) 
{
	if (count_q_repeats(ACT_TUSK_BLUR)>=crd_check[diff]) 
		d_block();
...
DMJ szc[]=
{
	...
	{FT_LK,lk_zap_low},
	{FT_RAYDEN,raiden_counter_zap},
	{FT_NOOB,d_propell_attack_now},
	{-1,NULL},
};
...
void raiden_counter_zap(void) 
{
	d_body_propell(PROP_RAIDEN_MELT);
	d_reaction_exit();
}


void c_noob_ego(void) 
{
	if (ask_mr_diff(rpt_promoves)==SYSTEM_CARRY_CLR) 
	{
		return_to_beware;
...
void wait_proj_pass(void)
{
	...
	do
	{
		...
		{
			current_proc->a2=-((long)current_proc->a2);
			current_proc->a4=-((long)current_proc->a4);
		}

		/* wpp5 */
		current_proc->a3+=((long)current_proc->a2);
		current_proc->a3-=((long)current_proc->a4);		// tail end of proj scan
		current_proc->a2=(current_proc->pa8)->oxpos.u.intpos;
	...
	while(((long)current_proc->a2)>((long)current_proc->a3) );
...
short his_proj_front_x(void)
{
	...
	current_proc->a3=ta5->oxpos.u.intpos;
	current_proc->a2=*ta0;		// a2 = x offset (stk table)
	current_proc->a4=*(ta0+2);
	if (ta5->oflags & M_FLIPH)
	{
		current_proc->a2=-((long)current_proc->a2);
		current_proc->a4=-((long)current_proc->a4);
	...
	(long)current_proc->a3+=(long)current_proc->a2;
	(long)current_proc->a3+=(long)current_proc->a4;
...
//		d_beware();
	...
	if (ask_mr_diff(rpt_cornered)==SYSTEM_CARRY_SET)
	{
		if (random() & 0x00800000)
			nr_uppercut_if_u_can();
		else nr_sweep_if_u_can();
	}
...
/******************************************************************************
 ...
 Description:	choose sk charge response
******************************************************************************/
JUMPTBL cav_skdash[]=
{
	d_block,
...
void c_sk_charge(void)
{
	react_jump_table_act(cav_skdash);
	return;
}
...
/* drone beginnings */
#define purple_stance do_amode_combo
...
SCENERIO tbl_scenerio1[]=
{
	{BKGD_BRIDGE_MOD,FT_RAIN,FT_JADE,SCX(0x30),SCX(0x130),do_rain_bolt,0,0x24},
	{BKGD_CAVE_MOD,FT_KANO,FT_SONYA,SCX(0x50),SCX(0x120),do_kano_pyscho,NULL,0x2d},
	{BKGD_SKTOWER_MOD,FT_NOOB,FT_JCAGE,SCX(0x80),SCX(0x120),do_noob_slam,NULL,0x2d},
	{BKGD_CITY_MOD,FT_ROBO1,FT_SWAT,SCX(0X70),SCX(0x110),0,do_swat_bombshilo,0x28},
	{BKGD_TEMPLE_MOD,FT_RAYDEN,FT_INDIAN,SCX(0x100),SCX(0x148),do_amode_combo,0,0x40},
	{BKGD_TOWER_MOD,FT_RAIN,FT_JADE,SCX(0x100),SCX(0x120),do_roundhouse,0,0x19},
	{BKGD_DESERT_MOD,FT_SG,FT_SWAT,SCX(0xa0),SCX(0x130),0,do_swat_gun,0x30},
	{BKGD_KUNGFU5_MOD,FT_BARAKA,FT_NOOB,SCX(0x70),SCX(0x120),0,do_noob_zap,0x25},
	{BKGD_FOREST_MOD,FT_REPTILE,FT_RAYDEN,SCX(0x50),SCX(0x120),0,do_reverse_throw_lightning,0x34},
	{BKGD_ARMORY_MOD,FT_LIA,FT_ERMAC,SCX(0x60),SCX(0x100),0,do_mystic_drop,0x26},
	{BKGD_MONK_MOD,FT_BARAKA,FT_JCAGE,SCX(0x80),SCX(0xa0),do_shred,0,0x38},
	{BKGD_MK2PIT_MOD,FT_RAYDEN,FT_BARAKA,SCX(0x50),SCX(0x80),do_raiden_shocker,0,0x30},
	{BKGD_GRAVE_MOD,FT_JCAGE,FT_OLDSMOKE,SCX(0x50),SCX(0x120),do_jc_zaplo,0,0x24},
	{BKGD_DEADPOOL_MOD,FT_LAO,FT_OLDSZ,SCX(0x80),SCX(0x100),0,do_sz_decoy,0x18},
	{BKGD_STREET_MOD,FT_BARAKA,FT_SCORPION,SCX(0x90),SCX(0xc0),do_sa_spin,0,0x26},
	{BKGD_KUNGFU2_MOD,FT_ST,FT_RAYDEN,SCX(0X30),SCX(0x140),do_st_summon,0,0x40},
	{-1}
};

void do_amode_combo(void) 
{
	d_run_a11(0x40,SCX(0x40));
	d_elbow();
