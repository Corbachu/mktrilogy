...
void noob_zap_flash(void) 
{
	...
	else current_proc->pdata.p_store8=20;
...
void r_spark(void) 
{
	current_proc->pdata.p_hitby=ACT_SPARK;
	zap_stumble();
}

void r_jc_snot(void) 
{
	current_proc->pdata.p_hitby=ACT_JCSNOT_LO;
	zap_stumble();
}

void r_jc_shoruken(void) 
{
	rsnd_big_smack();
	...
	flight(current_proc->pa8,SCX(0x38000),-SCY(0xd0000),SCY(0x8000),5,ANIM_KDOWN);
...
void r_jc_shadow(void) 
{
	reaction_start(REACT_START,generic_airborn_hit,4,NULL);
...
void r_sa_swipe(void) 
{
	current_proc->pdata.p_hitby=ACT_SWIPE;

	create_blood_proc(BLOOD_FACE2);			//-DHS- correct blood routine

	his_ochar_sound(1);
	
	reaction_start(REACT_START,airborn_hit_no_sound,2,NULL);
...
void r_spin(void) 
{
	current_proc->pdata.p_hitby=ACT_SA_SPIN;
	...
	if ((current_proc->pa8)->ochar==FT_SK)
		get_char_ani(ANIM_TABLE1,ANIM_SK_SHREDDED);
	else get_char_ani(ANIM_TABLE1,ANIM_SHREDDED);
	...
	do
	{
		if (--current_proc->a10==0)
		{
			...
			borrow_ochar_sound(0x0,FT_BARAKA);
			his_group_sound(2);
			current_proc->a10=0x8;
		}
	
		process_sleep(2);

		damage_to_me(1,current_proc->pa8);

		next_anirate();
	}
	while(get_his_action(current_proc)==ACT_SA_SPIN);
	
	flight(current_proc->pa8,SCX(0x60000),-SCY(0x50000),SCY(0x8000),5,ANIM_KDOWN);	//-DHS- boss ani fix
	land_on_my_back();
#if 0
	animate_a0_frames(2,ANIM_BFLIP);
	reaction_exit(current_proc->pa8);
#endif	
}
...
void r_shred(void) 
{
	...
	if (get_x_dist<SCX(0x50)) 
	{
		...
		multi_adjust_xy(current_proc->pa8,-SCX(0x30),-SCY(0x20));
	} 
	...
	do
	{
		...
		(current_proc->pa8)->oyvel.pos=(distance_from_ground(current_proc->pa8)<=SCY(0xb0))?0:SCY(0x10000);

		next_anirate();
	}
	while(get_his_action(current_proc)==ACT_SHRED);
...
void r_dive(void) 
{
	...
	delete_slave();
	current_proc->pdata.p_hitby=ACT_DIVE;

	current_proc->a11=0x60;
	do
	{
		process_sleep(2);
		if ((current_proc->pdata.p_otherproc)->p_comp_flag!=PC_RAIDEN_DRAG)
			break;
	}
...
void r_post_dived(void) 
{
	...
	get_char_ani(ANIM_TABLE1,ANIM_KDOWN);		// -DHS- boss pick ani knockdown
	mframew(5);
	mframew(5);
	getup_local_reaction_exit;
}

extern void *ochar_froze_pals[];
void r_lightning(void) 
{
	current_proc->pdata.p_hitby=ACT_THROW_BOLT;

	reaction_start(REACT_START,airborn_hit_no_sound,2,NULL);
	away_x_vel(current_proc,current_proc->pa8,SCX(0x30000));

	group_sound(2);		// wasted voice

	current_proc->a10=(ADDRESS)ochar_froze_pals[(current_proc->pa8)->ochar];
...
void raiden_getup_pal(void) 
{
	if ((current_proc->pa8)->ochar==FT_RAYDEN) 
	{
		get_char_ani(ANIM_TABLE1,ANIM_GETUP);
		player_swpal(current_proc,RD_ELEC_P,current_proc->pa8);		
...
void lightning_stumble(void) 
{
	get_char_ani(ANIM_TABLE1,ANIM_STUMBLE);
	init_anirate(2);
	...
	while (1)
	{
		...
		while(--current_proc->a11>0);
		current_proc->a11=5;
		if (current_proc->pdata.p_flags & PM_ALT_PAL)
			player_normpal();
		else player_froze_pal();
	}
...
void b_spin(void) 
{
	reaction_start(REACT_BLOCK,NULL,2,NULL);
	damage_to_me(3,current_proc->pa8);
	...
	do
	{
		...
		block_shake(ANIM_STANDBLOCK,2,2);
		process_sleep(2);
		next_anirate();
	}
	while(get_his_action(current_proc)==ACT_SA_SPIN);
...
void spear0(WORD pa0)
{
	...
	do
	{
		current_proc->p_comp_flag=PC_RSPEAR;
		process_sleep(2);
		current_proc->p_comp_flag=PC_CLEAR;
	}
...
void r_scream(void)
{
	...
	do
	{
		...
		obj->oxpos.u.intpos+=(long)current_proc->a11;
		(long)current_proc->a11=-(long)current_proc->a11;
...
void r_combo5(void)
{
	if ((current_proc->pa8)->ochar!=FT_LK && (current_proc->pa8)->ochar!=FT_JCAGE)
		set_half_damage(current_proc->pa8);
...
extern void *armory_module[];
extern void *tomb_module[];

extern void *dlists_subway_to_street[];
extern void *dlists_soul_to_tower[];
extern void *dlists_hell_to_cave[];
extern void *dlists_lair_to_armory[];
extern void *dlists_armory_to_tomb[];

void r_uppercut(void)
{
	if (current_proc->a10==0xedb00 || current_proc->a10==0xdedb00)
		reaction_start(REACT_START,NULL,4,NULL);
	...
	if (current_proc->a10==0xedb00)
	{
		...
		{
			case BKGD_PIT_MOD:
				gstate=GS_PITFALLSPEC;				// new game state
				fall_down_pit();
				break;
			case BKGD_BELL_MOD:
			...
			case BKGD_HADES_MOD:
				fall_in_lava();
				break;
			case BKGD_DEADPOOL_MOD:		// slime time
				r_fall_in_slime();
				break;
			case BKGD_TOMB_MOD:			// spiked time
				r_spike_rise();
		}
	}

	if (is_master_in_finish()==SYSTEM_CARRY_SET && curback==BKGD_KUNGFU5_MOD && f_thatsall==1) 
		fall_down_mk1pit();
	...
	if (gstate!=GS_AMODE && get_my_strength(current_proc->pa8)!=0 && process_exist(PID_DARK,-1)==NULL) 	// no blast in dark
	{
		if (process_exist(PID_REGENERATE,-1)==NULL && process_exist(PID_SWITCHEROO,-1)==NULL)			// only blast if not regenerate or switcheroo
		{
			...
			{
				case BKGD_SUBWAY_MOD:
					blast_entry(0xe4,street_module,dlists_subway_to_street,BKGD_STREET_MOD);		// blast_through_subway
					break;
				case BKGD_SOUL_MOD:
					blast_entry(SCY(0x125),tower_module,dlists_soul_to_tower,BKGD_TOWER_MOD);				// blast_through_soul
					break;
				case BKGD_HADES_MOD:
					blast_entry(SCY(0x125),cave_module,dlists_hell_to_cave,BKGD_CAVE_MOD);				// blast_through_hell
					break;
				case BKGD_LIAR_MOD:
					blast_entry(SCY(0x125),armory_module,dlists_lair_to_armory,BKGD_ARMORY_MOD);				// blast_through_lair
					break;
				case BKGD_ARMORY_MOD:
					blast_entry(SCY(0x125),tomb_module,dlists_armory_to_tomb,BKGD_TOMB_MOD);				// blast_through_armory
					break;
			}
		}
	}
...
{
	if ((current_proc->pa8)->oflags2 & M_DEAD)
		wait_forever();
...
void r_tusk_blur(void)
{
	...
	dict=obj->odict;											// save our dict ptr
	
	obj->oheap=0;							// set correct heap and dict
	obj->odict=top_dict;
	...
	obj->odict=dict;							// restore our dict
	...
	{
		process_sleep(2);
		obj->oheap=0;							// set for the correct heap, other dudes
		obj->odict=top_dict;							// set for the correct heap, other dudes
...
void reaction_start_chores(void)
{
	...
	if ((current_proc->pdata.p_flags & PM_SITDUCK) != 0 && f_death!=FAT_BRUTAL)	
...
WORD getup_speeds[]= {4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4};		//-u64-
...
void gup2(WORD ani)
{
	...
	/* is_stick_down */
#if SHAO_PLAY==1
	if ((JOYSTICK_IN_A0(current_proc) & MASK_JDOWN) && ((current_proc->pa8)->ochar!=FT_SK || (current_proc->pa8)->ochar!=FT_MOTARO))
#else
...
#if SHAO_PLAY==1
			if (ani!=ANIM_SWEEPUP && ((current_proc->pa8)->ochar!=FT_SK || (current_proc->pa8)->ochar!=FT_MOTARO))
#else
			if (ani!=ANIM_SWEEPUP)
#endif
			{
...
void blast_init(long pa4,void *pa5,void *pa6,long pa7)
{
	...
	if (p_agressor_active[0]==AGRESS_WAIT)
		p_agressor_active[0]=AGRESS_NORMAL;

	if (p_agressor_active[1]==AGRESS_WAIT)
		p_agressor_active[1]=AGRESS_NORMAL;
	
	immune_a0(current_proc->pdata.p_otherproc);		// dont kill otherguy
	immune_a0(exprc_er(PID_MASTER));
	
	pa0=process_exist(PID_AGRESSOR1,0xffff);
	if (pa0)
		immune_a0(pa0);
	
	pa0=process_exist(PID_AGRESSOR2,0xffff);
	if (pa0)
		immune_a0(pa0);
	...
	pa0=process_exist(PID_AGRESSOR1|0x8000,0xffff);
	if (pa0)
		unimmune_a0(pa0);
	pa0=process_exist(PID_AGRESSOR2|0x8000,0xffff);
	if (pa0)
		unimmune_a0(pa0);
...
void r_combo0(void)
{
	...
	if (f_death==FAT_BRUTAL)
		wait_forever();
	else reaction_exit(current_proc->pa8);
}
...
void combo1(void)
{
	...
	if (f_death==FAT_BRUTAL)
		away_x_vel(current_proc,current_proc->pa8,SCX(0x10000));
	else away_x_vel(current_proc,current_proc->pa8,SCX(0x20000));
...
void combo43(void)
{
	...
	boss=9;
	if (((current_proc->pa8)->ochar==FT_MOTARO || (current_proc->pa8)->ochar==FT_SK) && am_i_joy()==SYSTEM_CARRY_SET)
		boss=0;
		
	reaction_start(REACT_START,NULL,boss,NULL);
	set_no_block(current_proc->pa8);
...
//******************************************************************************/
//******************************************************************************/
//-DHS- move to correct background file once works
void r_super_uppercut(void)
{
	wait_forever();
}
