...
JUMPTBL projectile_jumps [] =
{
	...
	do_proj_sa_zap,			// 2d			//-u64-
	do_proj_throw_bolt,		// 2e			//-u64-
	do_proj_noob_zap,		// 2f			// -u64-
	do_proj_rain_zap,		// 30	//-u64-
	do_proj_swat_bombshilo,		// 31
	do_proj_swat_bombslohi,		// 32
	do_proj_robo_2zap,		// 33
	do_proj_jc_zaphi,		// 34
	do_proj_jc_zaplo,		// 35
	do_proj_reverse_bolt,	// 36
	do_proj_lia_forward_2zap,	// 37
};

void reflect_blob_snake(void);
...
void rain_movement(void)
{
	...
	do
	{
		...
		/* check joy stick */
		if ( current_proc->a10==0 && ( (JOYSTICK_IN_A0(pthrower) & MASK_JUP)  || (((pthrower->pdata.p_flags & PM_JOY)==0) && randper(500)==SYSTEM_CARRY_SET)) )
		{
			/* going up */
			ta10=4;
...
void do_proj_sa_zap(void) 
{
	...
	zap_init(ZINIT_SPEC_ACT,ACT_SPARK,0,obj);
	ochar_sound(2);
	animate_a9(3,ANIM_ZAP);
	stk_a9=current_proc->pa9;

	get_char_ani(ANIM_TABLE1,ANIM_PROJECTILE);		// MOD TO GET SK PROJECTILE ART
	get_proj_obj_m(obj);

	current_proc->a11=(ADDRESS)current_proc->pa9;
	multi_adjust_xy((OBJECT*)current_proc->a10,0,-SCY(0xc));
	insert_object((OBJECT *)current_proc->a10,&objlst);

	current_proc->pa9=stk_a9;
	double_mframew(4);

	create_proj_proc(spark_proc);
	process_sleep(0x1c);
	do_next_a9_frame(obj);
	process_sleep(4);
...
void spark_proc(void) 
{
	...
	do_next_a9_frame(obj);
	if (proj_strike_check(obj,0x16)==SYSTEM_CARRY_CLR) 	
	{
		process_sleep(3);
		do_next_a9_frame(obj);
		if (proj_strike_check(obj,0x16)==SYSTEM_CARRY_CLR) 	
		{
			...
			projectile_flight_call(current_proc->pa8,0x17);
		}
	}

	/* spark_hit */
	dead_projectile(current_proc);
	ochar_sound((f_block)?4:3);
	...
	match_me_with_him();
	flip_multi(obj);
	multi_adjust_xy(obj,-SCX(0x80),SCY(0));
...
void throw_bolt(WORD pa10) 
{
	...
	if (pa10==1) 
	{
		zap_init(ZINIT_SPEC_ACT,ACT_REVERSE_BOLT,0,obj);
	...
	{
		zap_init(ZINIT_SPEC_ACT,ACT_THROW_BOLT,0,obj);
	}
	...
	((OBJECT*)current_proc->a10)->ozval=obj->ozval+1;
	...
	create_proj_proc(lightning_proc)->a10=pa10;
...
void lightning_proc(void) 
{
	...
	tell_world_stk((pa10==0)?0x11:0x13);
	
	find_ani_part2(ANIM_PROJECTILE);
	mframew(2);
	
	if (light_pre_check((pa10==0)?0x10:0x12)==SYSTEM_CARRY_SET)
		lightning_hit(pa10);
	if (light_pre_check((pa10==0)?0x10:0x12)==SYSTEM_CARRY_SET)
		lightning_hit(pa10);
	...
	current_proc->a6=(pa10==0)?NULL:reverse_border_check;
	projectile_flight_call(obj,(pa10==0)?0x11:0x13);
	lightning_hit(pa10);
...
void reverse_border_check(void) 
{
	...
	{
		current_proc->pdata.p_store8--;
		if (current_proc->pdata.p_store8==0)
			current_proc->pdata.p_store1=NULL;
	}
	else
	{
		...
		{
			if (obj->oxvel.pos>0) 
				obj->oxpos.u.intpos=worldtlx.u.intpos-obj->osize.u.xpos;
			else obj->oxpos.u.intpos=worldtlx.u.intpos+SCRRGT+obj->osize.u.xpos;
			
			current_proc->pdata.p_store8=10;
		}
	}
...
void lightning_hit(WORD pa10) 
{
	...
	if (f_block)
		borrow_ochar_sound(4,FT_RAYDEN);
	else borrow_ochar_sound(3,FT_RAYDEN);
	...
	do
	{
		...
		obj->oxvel.pos=other->oxvel.pos;
		obj->oyvel.pos=other->oyvel.pos;
	}
	while(1);
}
...
WORD light_pre_check(WORD stk) 
{
	...
	process_sleep(1);
	if (proj_strike_check(current_proc->pa8,stk)==SYSTEM_CARRY_SET)
		q_yes;
...
void do_proj_ermac_zap(void)
{
	...
	get_char_ani(ANIM_TABLE2,ANIM_ERMAC_ZAPPER);		
...
void ermac_zap_proc(void)
{
	...
	current_proc->a10=(long)(obj->oxpos.u.intpos);
	obj->oxpos.u.intpos=(short)current_proc->a7;

	current_proc->a11=(long)(obj->oypos.u.intpos+SCY(0x40));
...
void floor_ice_hit(void) 
{
	...
	(current_proc->pdata.p_otherproc)->a10=leftmost_mpart(obj);
	(current_proc->pdata.p_otherproc)->a11=rightmost_mpart(obj);
...
void do_proj_mileena_zap(void) 
{
	...
	{
		/* mileena_air_zap */
		zap_init(ZINIT_AIR,ACT_MIL_AIR_ZAP,0,obj);
		current_proc->pdata.p_action=ACT_MIL_AIR_ZAP;
...
void ssp3(short pa0,short pa1)
{
	...
	oa11->ochar=((OBJECT *)current_proc->a11)->ochar;
	tell_world_stk(0x14);
	
	current_proc->pa8=oa11;
	find_ani2_part_a14(ANIM_SCORP_THROWS,4);
	oa10=(OBJECT*)current_proc->a10=get_proj_obj_m(oa11);
	current_proc->pa8=pa8;
	oa10->header.t_width=0;			// NULL OBJECT
	oa10->header.t_height=0;
	insert_object((OBJECT *)current_proc->a10,&objlst);

	((OBJECT *)current_proc->a10)->ochar=oa11->ochar;
	((OBJECT *)current_proc->a10)->oxvel.pos=pa8->oxvel.pos;

	match_ani_points(oa11,oa10);
	...
	*/
	...
	do
	{
		ta0=abs(pa8->oxpos.u.intpos-oa11->oxpos.u.intpos);	// distacen from throw tip to thrower (A)

		ta3=(oa11->ochar==FT_SCORPION)? 234:250;		// set clip value

		ta3=abs(ta3-ta0);			// a3 amount we want 2 clip

		if (oa10->oflags & M_FLIPH) 
		{
			/* ssp6 */
			ta2=oa10->ofset;			// old clip setting
			oa10->ofset=ta3;
			ta3-=ta2;					// clip change
			oa10->oxpos.u.intpos-=ta3;	// remove
		}
		else
		{
			/* ssp8 */
...
//	temp=abs(p1_obj->oxpos.u.intpos-p2_obj->oxpos.u.intpos);
//	if ((oa10->oflags & M_FLIPH))
//		oa10->oxpos.u.intpos=obj->oxpos.u.intpos-temp+(oa10->ofset & 0xff);

	lineup_rope_with_him();
	pull_clip();

	process_sleep(2);
	if ((current_proc->pdata.p_otherproc)->p_comp_flag!=PC_RSPEAR)
		goto SPEAR_CANCEL;
	...
	do
	{
		...
		if ((oa10->oflags & M_FLIPH)==0)
			oa10->oxpos.u.intpos=obj->oxpos.u.intpos;
//		else oa10->oxpos.u.intpos=obj->oxpos.u.intpos-temp+oa10->ofset;
...
void clip_scorpion_rope(short pa3,short pa4) 
{
	...
	ta5=obj->oxpos.u.intpos;

	if (obj->oflags & M_FLIPH) 
	{
		ta5+=pa3;
		//ta5+=(obj->osize.u.xpos-pa4);		
		ta5-=pa4;
	}
	else
	{
...
void lineup_rope_with_him(void)
{
	...
	current_proc->a7=oa10->oypos.u.intpos;
...
void get_single_spear_obj(void) 
{
	...
	gso_dmawnz(obj,(ADDRESS)COMPUTE_ADDR(pa8->oheap,GET_LONG(current_proc->a11)),obj->odict,obj->oheap,0);
	alloc_cache((OIMGTBL *)COMPUTE_ADDR(obj->oheap,GET_LONG(current_proc->a11)),obj->oheap,obj);
	(OBJECT *)current_proc->a10=obj;
	obj->oflags|=M_CLIP;				// set clipping on for spear
	obj->ofset=0;
	insert_object(obj,&objlst);
	current_proc->pa8=pa8;
	lineup_1pwm(obj,pa8);
...
void spit_prezap(short pa1x,short pa1y,short pa2x,short pa2y) 
{
	...
	if (strike_check_box(current_proc->pa8,0x12,pa1x,pa1y,pa2x,pa2y)==SYSTEM_CARRY_SET && is_jade_protected()==SYSTEM_CARRY_CLR)
		spit_prezap_hit();
	return;
}
...
{
	update_tsl(l_orb_slow,current_proc->pa8);
...
void fan_proc(void) 
{
	...
	set_proj_vel(obj,SCX(0x80000),2);
...
void do_proj_motaro_zap(void)
{
	...
	zap_init(ZINIT_SPEC_ACT,ACT_MOT_ZAP,0,obj);
...
void do_proj_sk_zap(void)
{
	...
	zap_init(ZINIT_SPEC_ACT,ACT_SK_ZAP,0,obj);

	animate_a9(3,ANIM_ZAP);
	ta9=current_proc->pa9;
	slave=current_proc->pdata.p_slave;
	...
	current_proc->pdata.p_slave=slave;
	delete_slave();
...
void sk_zap_proc(void)
{
	...
	current_proc->a10=obj->oxpos.u.intpos;
	obj->oxpos.u.intpos=(short)current_proc->a7;
...
void saw_strike_check(void)
{
	...
	while(q_is_he_reacting()==SYSTEM_CARRY_SET && ((((PROCESS *)(current_proc->pdata.p_store1))->pdata.p_otherproc)->pdata.p_flags & PM_CORPSE)==0);
...
void lia_forward_zap(WORD ta11)
{
	...
	pa0->a11=(current_proc->a11==1)?SCX(0xa0000):SCX(0x80000);
	...
	current_proc->pdata.p_action=ACT_PROJ_SD;
	process_sleep((ta11==1)?0x30:0x20);
...
void sz_zap_hit(void)
{
	...
	if ((current_proc->pa8)->ochar==FT_OLDSZ)
		find_ani2_part_a14(ANIM_OSZ_ZAP,5);
	else find_ani_part_a14(ANIM_ZAP,3);
...
******************************************************************************/
void swat_bomb_launch(OBJECT *obj) 
{
	OBJECT *obomb;
	void *ta9;
...
{
	zap_init(ZINIT_SPEC_ACT,ACT_SWAT_BOMB_HILO,0,current_proc->pa8);
	current_proc->a11=1;
	stack_jump(do_proj_swat_bombs);
}
...
void sg_zap_trail(void)
{
	...
	gso_dmawnz(obj,GET_LONG(current_proc->pa9),explode_dict,0,0);
	alloc_cache((OIMGTBL *)GET_LONG(current_proc->pa9),0,obj);
	lineup_1pwm(obj,(OBJECT *)current_proc->a11);

	obj->ozval=1;
	
	obj->oflags|=M_XLU_ON;
	obj->oflags|=THREE_QUARTER_XLU;
...
void rzap3(FUNC routine,WORD count)
{
	OBJECT *obj=current_proc->pa8;
	...
	process_sleep((count==1)?0x30:0x20);
...
void rocket2_proc(void)
{
	...
	current_proc->pdata.p_store1=rocket_routines[0].r_accelcnt;
	...
	gso_dmawnz(objtarget,target,obj->odict,obj->oheap,0);			// create target
...
void skull_proc(void)
{
	...
	if ((current_proc->pdata.p_otherguy)->ochar!=FT_MOTARO) 		// motaro = reflect
	{
		if (get_his_action(current_proc)!=ACT_REFLECT)
		{
			if (local_strike_check_box(0x11,SCX(0x57),SCY(0x15),SCX(0x52),SCY(0x16))==SYSTEM_CARRY_SET && ((current_proc->pdata.p_otherproc)->pdata.p_flags & PM_CORPSE)==0)
			{
				/* pre_skull_hit */
...
/******************************************************************************
 Function: void stuff_others_a0(PROCESS *pa0)
...
void do_proj_ind_zap(void)
{
	...
	rflag=q_his_react_flag_set();
	animate_a9((rflag==SYSTEM_CARRY_SET)?1:3,ANIM_ZAP);

	obj_slave=current_proc->pdata.p_slave;
	a10=current_proc->a10;
	setup_proj_obj(obj);
	if (rflag==SYSTEM_CARRY_SET)
		swpal(REDFLM_P,(OBJECT *)current_proc->a10);
...
void doice5(void)
{
	...
	animate2_a9(0x0003,ANIM_OSZ_SKYZAP);
	...
	backwards_ani2(3,ANIM_OSZ_SKYZAP);
...
void sky_ice_proc(void)
{
	...
	{
		process_sleep(1);
		if (proj_strike_check(obj,0x1e)==SYSTEM_CARRY_SET)
		{
			/* ice_hit */
			hob_ochar_sound(0x0002+0x13,0x0003+0x13);
...
WORD projectile_flight_call(OBJECT *obj,WORD strkoff)
{
	...
	do
	{
		...
		/* pflt5 */
		if (proj_onscreen_test(obj)==SYSTEM_CARRY_CLR && current_proc->pdata.p_store1!=(ADDRESS)reverse_border_check)
			delete_proj_and_die(obj);
...
void benedict_arnold_projectile(OBJECT *obj)
{
	...
	if (obj->oflags2 & M_MULTIPART)
		flip_multi(obj);
	else flip_single(obj);
	
	current_proc->a4=get_frontmost_point(obj);
	ta3=-abs((short)current_proc->a4-ta3);		// we ALWAYS want negative here

	current_proc->a4=rightmost_mpart(obj);
...
WORD q_is_he_noob_zap(void)
{
	...
	pid=(current_proc->pdata.p_otherguy==p1_obj)?PID_NOOB_ZAP_FLASH1:PID_NOOB_ZAP_FLASH2;
	pa0=process_exist(pid,-1);
...
/******************************************************************************
 Function: void i_am_a_sitting_duck(void)
...
OBJECT *get_proj_obj_m(OBJECT *obj)
{
	...
	ptemp=gmo_proc(obj->odict,obj->oheap);				// get process which holds data
...
{
	zap_init(ZINIT_SPEC_ACT,ACT_JCSNOT_HI,0,current_proc->pa8);
	jczap4(-SCY(0xa0000),SCX(0xa0000),SCY(0x8000));
	
}

void  do_proj_jc_zaplo(void) 
{
	zap_init(ZINIT_SPEC_ACT,ACT_JCSNOT_LO,0,current_proc->pa8);
	jczap4(-SCY(0x60000),SCX(0xc0000),SCY(0x6000));
...
void jczap4(long pa0,long pa1,long pa2) 
{
	...
	ptemp=create_proj_proc(jc_snot_proc);
	ptemp->a10=pa0;
	ptemp->a11=pa1;
	ptemp->pa9=pa2;
...
void jc_snot_proc(void) 
{
	...
	multi_adjust_xy(obj,SCX(0x20),SCY(0x40));
	...
	ta5=(OIMGTBL *)COMPUTE_ADDR(obj->oheap,GET_LONG(current_proc->a10));
	ta5=(OIMGTBL *)COMPUTE_ADDR(obj->oheap,GET_LONG(ta5));
	
	current_proc->a11=get_snot_trail_oid();
	
	current_proc->a10=(ADDRESS)obj;		// master blob piece
	current_proc->pdata.p_store1=(ADDRESS)obj->oslink;
	
	current_proc->a6=6;
	/* snot2 */
	do
	{
		current_proc->a7=(ADDRESS)current_proc->pa8;
		gso_dmawnz(obj,(ADDRESS)ta5,obj->odict,obj->oheap,0);
		alloc_cache(ta5,obj->oheap,obj);
		animate(SINGLE_ANI0,(OIMGTBL *)COMPUTE_ADDR(obj->oheap,GET_LONG(current_proc->pa9)),obj->oflags);
		((ADDRESS*)current_proc->pa9)++;
		
		obj->oid=current_proc->a11;
		
		/* center x,y */
		if (((OBJECT*)current_proc->a10)->oflags & M_FLIPH)
		{
//			flip_single(obj);
			obj->oxpos.u.intpos=((OBJECT*)current_proc->a10)->oxpos.u.intpos+((OIMGTBL *)obj->oimg)->anixoff-obj->osize.u.xpos;
		}	
		else obj->oxpos.u.intpos=((OBJECT*)current_proc->a10)->oxpos.u.intpos-((OIMGTBL *)obj->oimg)->anixoff;
		
		obj->oypos.u.intpos=((OBJECT*)current_proc->a10)->oypos.u.intpos-((OIMGTBL *)obj->oimg)->aniyoff;
		
		/* behind prev */
		obj->ozval=((OBJECT*)current_proc->a7)->ozval-1;
		
		insert_object(obj,&objlst);
		
		(OBJECT*)((OBJECT *)current_proc->a7)->oslink=obj;
	}
	...
	/* set arc a0=a9 a1=a10 a2=a11*/
	current_proc->a11=ta9;
	obj->oyvel.pos=ta10;
	set_proj_vel(obj,ta11,3);
	...
	do
	{
		...
		if (((current_proc->pdata.p_otherguy)->ochar==FT_MOTARO || get_his_action(current_proc)==ACT_REFLECT) && 	// motaro reflects
		   (strike_check_a0_test(obj,0x12)==SYSTEM_CARRY_SET))
		{
			reflect_blob_snake();
			obj->oyvel.pos=ta10/2;
		}
		else
		{
		...
		((OBJECT*)current_proc->a10)->oyvel.pos+=ta9;
		process_sleep(1);
		update_blob_snake();
	}
	while(proj_onscreen_test(obj)==SYSTEM_CARRY_SET);
	
	dallobj(get_snot_trail_oid());
	delete_master_snot();
}
...
{
	(OBJECT *)(current_proc->pa8)->oslink=(OBJECT*)current_proc->pdata.p_store1;
	delete_proj_and_die(current_proc->pa8);
}
...
void update_blob_snake(void) 
{
	...
	ta0=(OBJECT*)current_proc->a10;
	ta1=(OBJECT *)ta0->oslink;
...
void reflect_blob_snake(void) 
{
	...
	projectile_reflect(current_proc->pa8);

	ta1=(OBJECT *)ta0->oslink;
	do
	{
		if (((OBJECT*)current_proc->a10)->oflags & M_FLIPH)
			ta1->oxpos.u.intpos=((OBJECT*)current_proc->a10)->oxpos.u.intpos+((OIMGTBL *)ta1->oimg)->anixoff-ta1->osize.u.xpos;
		else ta1->oxpos.u.intpos=((OBJECT*)current_proc->a10)->oxpos.u.intpos-((OIMGTBL *)ta1->oimg)->anixoff;
		
		ta1->oypos.u.intpos=((OBJECT*)current_proc->a10)->oypos.u.intpos-((OIMGTBL *)ta1->oimg)->aniyoff;
...
{
	if ((current_proc->pdata.p_otherguy)->oid!=PID_P1)
		return(OID_SNOT_TRAIL1);
	else return(OID_SNOT_TRAIL2);
}
