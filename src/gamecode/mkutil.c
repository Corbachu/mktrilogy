...
{
	sys_memset(&(current_proc->pdata),0,sizeof(PDATA));
	return;
}
...
******************************************************************************/
void agressor_increaser(WORD avalue,WORD player,OBJECT *obj) 
{
#if 0
	if (obj->oflags2 & M_QUARTER_DAMAGE)
		avalue=avalue/8;
	else
	{
		if (obj->oflags2 & M_HALF_DAMAGE)
			avalue=avalue/4;
	}	
#endif	
	p_agressor[player]+=avalue;
...
void inc_his_p_hit(void)
{
	...
	player=(current_proc->pdata.p_otherguy==p1_obj)?1:0;
	
	if (p_agressor_active[player]==AGRESS_NORMAL)
	{
		hitby=(current_proc->pdata.p_otherproc)->pdata.p_hitby;
		if (hitby==ACT_HIPUNCH || hitby==ACT_LOPUNCH || hitby==ACT_COMBO)
		{
			if (hitby==ACT_COMBO)
				avalue=(AGRESSOR_HIT/2);
			else avalue=(AGRESSOR_HIT/4);
		}
		else avalue=(AGRESSOR_HIT/1);
		
		if (hitby!=ACT_BRUTAL)
			agressor_increaser(avalue,player,current_proc->pdata.p_otherguy);
	}
...
void inc_p_hit(void)
{
	...
	player=(current_proc->pa8==p1_obj)?1:0;
	
	if (p_agressor_active[player]==AGRESS_NORMAL) 
	{
		hitby=current_proc->pdata.p_hitby;
		if (hitby==ACT_HIPUNCH || hitby==ACT_LOPUNCH || hitby==ACT_COMBO  ) 
		{
			if (hitby==ACT_COMBO)
				avalue=(AGRESSOR_HIT/2);
		...
		if (hitby!=ACT_BRUTAL)
			agressor_increaser(avalue,player,current_proc->pa8);
	}
...
void inc_p_block(void ) 
{
	int player;
	short hitby;
	WORD avalue;
	
	current_proc->pdata.p_block++;
	...
	if (p_agressor_active[player]==AGRESS_NORMAL) 
	{
		...
		{
			if (hitby==ACT_COMBO)
				avalue=(AGRESSOR_BLOCK/3);
			else avalue=(AGRESSOR_BLOCK/4);
		}
		else avalue=(AGRESSOR_BLOCK/1);
...
WORD ishe_close_to_edge(PROCESS *mproc)
{
...
void murder_myoinit_score(void)
{
	...
	jade_pal=get_fore_pal(JSCORE_P);
	mil_pal=get_fore_pal(MSCORE_P);
	kit_pal=get_fore_pal(KSCORE_P);
	get_fore_pal(WHITE_p);
	return;
}
...
#if TEXT_QUE	
	tque_ptr=text_queue;				// init texture queue ptr
#endif
...
void fastxfer(PROCESS *pxfer,JUMPTBL jumpptr)
{
	...
	pxfer->p_context.c_frameptr=pxfer->p_context.c_stackptr=(ADDRESS)&(pxfer->p_context.c_stack[STKSIZE/8-3]);        //set start of stack for process
...
void koc2(OBJECT **olist,WORD id,WORD mask)
{
	...
	while (obj_ptr->olink != NULL)
	{
		...
		{
			/* remove from vcache system */
			dealloc_vram_mem(obj_ptr->ovcache);
			obj_ptr->ovcache=VCACHE_FAILED;
...
{
	delpal(obj);
	if ((obj->opal=get_fore_pal((WORD *)charpal))==0xffff)
		ermac(8);
...
/******************************************************************************
 Function: PROCESS *gmo_proc(BYTE *dict,void *heapptr)
...
******************************************************************************/
PROCESS *gmo_proc(BYTE *dict,void *heapptr)
{
	LONG animframe;
	animframe=GET_LONG(current_proc->pa9);
	animframe=(LONG)COMPUTE_ADDR(heapptr,animframe);

	get_multi_obj((ADDRESS *)animframe,0,dict,heapptr,PFLAG_PALLOAD);
	return(NULL);
}

/* user to support more then two fighters on screen */
PROCESS *gmo_proc_special(BYTE *dict,void *heapptr,WORD palflag)
{
	LONG animframe;
	animframe=GET_LONG(current_proc->pa9);
	animframe=(LONG)COMPUTE_ADDR(heapptr,animframe);

	get_multi_obj((ADDRESS *)animframe,0,dict,heapptr,palflag);
	return(NULL);
}
...
/******************************************************************************
 Function: OBJECT *get_multi_obj(ADDRESS *framelist,void *mram,BYTE *dict,void *heapptr,WORD getpal)
...
******************************************************************************/
OBJECT *get_multi_obj(ADDRESS *framelist,void *mram,BYTE *dict,void *heapptr,WORD getpal)
{
	OBJECT *obj;
	LONG frame;
...
******************************************************************************/
OBJECT *get_single_obj(LONG frame,BYTE *dict,void *heapptr,void *pal,WORD palflag)
{
	OBJECT *obj;

	//-NOTE- assumes code space start at 0x80000000
...
OBJECT *object_setup(OIMGTBL *frame,void *palram,WORD getpal)
{
	...
	if (getpal!=0)                          // check to get a pal
	{
		...
		if ((long)((frame+1)->isagptr)>=0x80000000 && (long)((frame+1)->isagptr)<=0x80400000)
...
/******************************************************************************
 Function: OBJECT *gbo_dmawnz(OIMGTBL *frame,BYTE *dict,void *heapptr,void *palram)
...
******************************************************************************/
OBJECT *gbo_dmawnz(OIMGTBL *frame,BYTE *dict,void *heapptr,void *palram)
{
	OBJECT *obj;
...
void delolist(OBJECT **obj_list)
{
	...
	while (obj!=NULL)
	{
		...
		dealloc_vram_mem(kilobj->ovcache);	//-NOTE- new add
...
volatile short randadj;
static short randadj2;
static short addend;
...
short nrand(void)
{
	...
	ret=(((randadj^((randadj2<<5)+(addend<<3)-1))-randadj2)^addend);
	addend++;
	if ( addend>randadj2 )
		addend++;
...
void back_to_shang_check(void)
{
	...
	if (q_switch_ok()==SYSTEM_CARRY_CLR)
		return;

	back_to_shang_form(CHAR_NORMAL);
}

/* test to see if need to keep current dude in memory due to other objects on screen */
WORD q_switch_ok(void) 
{
	PROCESS *ptemp;
	...
	if (process_exist(PID_NOOB_EGO,0xffff)!=NULL)                      // check blade and dead blade
		q_no;

	if (process_exist(PID_GRAVP,0xffff)!=NULL)                      // check blade and dead blade
		q_no;

	if (process_exist(PID_HAT_RING,0xffff)!=NULL)                       // check for sonic
		q_no;
	...
	if ( process_exist(PID_SUMMON,-1)!=NULL)
		q_no;
	
	if ( process_exist(PID_DEAD_BLADE,-1)!=NULL)		//-DHS- add code to check for correct player
		q_no;
	
	if ( process_exist(PID_BLADE,-1)!=NULL)		//-DHS- add code to check for correct player
		q_no;
	
	if ( process_exist(PID_BOOM_TRAIL,-1)!=NULL)		//-DHS- add code to check for correct player
		q_no;
	
	if (get_his_action(current_proc)==ACT_NOOGY)	
		q_no;
	...
	if ( (current_proc->pa8)->oid==OID_P1)
	{
		...
		if (process_exist(PID_FAN2,0xffff)!=NULL)                       // check for sonic
			q_no;

		if (process_exist(PID_P2ROCKET1,0xffff)!=NULL)                       // check for sonic
			q_no;

		if (process_exist(PID_P2ROCKET2,0xffff)!=NULL)                       // check for sonic
			q_no;
			
		if (process_exist(PID_P1_EGO,0xffff)!=NULL)                      // check blade and dead blade
			q_no;
	}
	else
	{
		...
		if (process_exist(PID_FAN1,0xffff)!=NULL)                       // check for sonic
			q_no;

		if (process_exist(PID_P1ROCKET1,0xffff)!=NULL)                       // check for sonic
			q_no;

		if (process_exist(PID_P1ROCKET2,0xffff)!=NULL)                       // check for sonic
			q_no;

		if (process_exist(PID_P2_EGO,0xffff)!=NULL)                      // check blade and dead blade
			q_no;
	}
...
void back_to_shang_form(WORD ltype)
{
	OBJECT *obj=current_proc->pa8;
	WORD ochar;
	void *animframe;
	...
	if ( ochar==FT_SMOKE || ochar==FT_OLDSMOKE  )
	{
		ptemp=active_head;
		while ( ptemp!=NULL )
		{
	...
	{
		back_shang_ani();
//only two frames on morph	back_shang_ani();
		back_shang_ani();
	}
	/* switch to shang pal */
//	obj->ochar=FT_ST;
...
OBJECT *make_solid_object(WORD pa0,WORD pa3x,WORD pa3y)
{
	...
	/* set color */
	obj->oconst=pa0;			// set color of solid object
...
void make_cursor_objects(WORD *ctbl,WORD oid,WORD color) 
{
	OBJECT *ta9;
	OBJECT *obj;
	
	ta9=obj_free;
	current_proc->a10=(ADDRESS)ta9;
	
	do
	{
		obj=make_solid_object(color,4,4);
		set_noscroll(obj);
		obj->oid=oid;
		
		obj->osize.u.xpos=*ctbl++;         // set size of box
		obj->osize.u.ypos=*ctbl++;
		(OBJECT*)ta9->oslink=obj;
		obj->oxpos.u.intpos=*ctbl++;
		obj->oypos.u.intpos=*ctbl++;
		insert_object(obj,&objlst);
		ta9=obj;
	}
	while(*ctbl!=0xffff);
	
	(OBJECT*)ta9->oslink=NULL;
