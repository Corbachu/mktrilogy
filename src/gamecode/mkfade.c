...
void single_pal_fader(void)
{
	...
	//-SONY_IGNORE- l=(((LONG)current_proc->pa9)-getClut(FORE_CLUT_START_X,FORE_CLUT_START_Y))>>CLUT_ID_SHIFT;
	...
	do
	{
		...
		//-SONY_IGNORE- palset_xfer(FORE_CLUT_START_X+((l%CLUT_FACTOR)<<CLUT_SHIFT_SIZE),FORE_CLUT_START_Y+(l/CLUT_FACTOR),(WORD *)(&(current_proc->p_context.c_stack)));
		palset_xfer(l,(WORD *)(&(current_proc->p_context.c_stack)));
...
void newfade(WORD *pa0,WORD *pa1)
{
	...
	*((LONG *)pa0)=ta14;					// write count
	...
	do
	{
		...
		ta3&=0x003e;				// blue
		ta5=(ta5 & 0x07c0)>>5;		// green
		ta7=(ta7 & 0xf800)>>5;		// red
...
void fadepal(WORD *pa0,WORD *pa1,WORD pa2)
{
	...
	*((LONG *)pa0)=ta14;					// write count
	...
	do
	{
		...
		ta7=(ta3 & PRE_A8)*pa2;		// blue
		ta5=(ta3 & PRE_A6)*pa2;		// green
		ta3=(ta3 & PRE_A4)*pa2;		// red
		...
		ta3|=high;					// 1st entry will be zero all others will have bit sets
		//-SONY-high=0x8000;
		high=0x0001;					// set alpha bit for u64
...
//	ta8=0x10000;
	...
	do
	{
		if (fadeout_mercy_tab[i]>96)
		{
			ta8=0x10000;
		}
		else
		{
			if (fadeout_mercy_tab[i]==64)
			{
				ta8=0x8000;
...
//	while(ta8>0x8000);
	while (fadeout_mercy_tab[i]!=0xffff);
...
WORD xcolor(WORD pa0,LONG pa1)
{
	...
	ta4=(((pa0 & 0x3e) * pa1) & 0x3e0000)>>16;		// blue

	ta3=(((pa0 & 0x7c0) * pa1) & 0x7c00000)>>16;		// green
	ta4|=(WORD)ta3;

	ta3=(((pa0 & 0xf800) * pa1) & 0xf8000000)>>16;		// red

	ta4|=(WORD)ta3;
	ta4|=0x0001;				// ENABLE ALPHA
