...
void get_initial_proc(void)
{
	...
	current_proc->pdata.p_joyport=(current_proc->pdata.p_flags==0)? (WORD*)&swcurr.u.p1pad:(WORD*)&swcurr.u.p2pad;
...
//-SONY_IGNORE- 	objbase->header.t_xoffset=0xff;			// set rgb color
//-SONY_IGNORE- 	objbase->header.t_yoffset=0x80;
//-SONY_IGNORE- 	objbase->header.t_width=0;
	objbase->oconst=0x00ff;				// set rgb color 
...
//-SONY_IGNORE-	obj->header.t_xoffset=0xff;			// set rgb color
//-SONY_IGNORE-	obj->header.t_yoffset=0x80;
//-SONY_IGNORE-	obj->header.t_width=0x0;
	obj->oconst=0x00ff;
...
char txt_rank_title[]="INCREDIBLE WINNING STREAKS";
