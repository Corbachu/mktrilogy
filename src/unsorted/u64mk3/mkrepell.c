...
void stuff_a6a7(void)
{
	...
	if (((long)current_proc->a5 > SCREEN_DISTANCE) && ((p1_obj->oflags2|p2_obj->oflags2) & M_NOEDGE)==0)
	{
		(long)current_proc->a5 -=SCREEN_DISTANCE;			// amount we need to adjust
		...
		if (p2_obj->oxpos.pos >= p1_obj->oxpos.pos)
		{
			...
			if ((long)current_proc->a5>3)
			{
				(long)current_proc->a5=((long)(current_proc->a5)>>1);		// take 1/2 error

				/* stuf9 */

				/* adjust p2 pos */
				p2_obj->oxpos.u.intpos +=(-((long)current_proc->a5));

				/* adjust p1 pos */
				p1_obj->oxpos.u.intpos +=((long)current_proc->a5);
			}
		}
		else
		{
...
REPELL:
		...
		current_proc->a4=(long)current_proc->a6;
		current_proc->a5=(long)current_proc->a7-GRACE_PIXELS;			// a4 = high pt : a5 = low point

		get_player_hilo(p2_obj);
		current_proc->a7=(long)current_proc->a7-GRACE_PIXELS;			// a6 = high pt : a7 = low point

		if ((long)current_proc->a5 < (long)current_proc->a6 || (long)current_proc->a7 < (long)current_proc->a4)
		{
			 /* player_above_player */
			 get_player_anis();
			 current_proc->a6=p1_xvel.pos;
			 current_proc->a7=p2_xvel.pos;
			 stuff_a6a7();
		}
		else
		{
			...
			if (b4==0)
			{
				...
				if ((long)current_proc->a5 < (REPELL_DISTANCE-3))
				{
					/* repell_now */
			...
			if ((((long)current_proc->a6=p1_xvel.pos) != 0) && ((long)current_proc->a5 < REPELL_DISTANCE) && (b4 == 0))
			{
				if ((long)current_proc->a6 >=0)
				{
			...
			if ((((long)current_proc->a7=p2_xvel.pos)!=0) && ((long)current_proc->a5<REPELL_DISTANCE) && (b4==0))
			{
				if ((long)current_proc->a7 >= 0)
				{
...
void get_player_hilo(OBJECT *obj)
{
	...
	current_proc->a6=highest;
	current_proc->a7=lowest;
...
{
	current_proc->a4=(long)abs(p1_obj->oypos.u.intpos-p2_obj->oypos.u.intpos);		// y distance

	current_proc->a5=(long)abs(p1_obj->oxpos.u.intpos-p2_obj->oxpos.u.intpos);		// x distance
}
