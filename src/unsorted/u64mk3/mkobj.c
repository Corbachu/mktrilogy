...
void object_initialize(void)
{
	...
	get_fore_pal(JSCORE_P);				// palette #2 score
	get_fore_pal(KSCORE_P);				// palette #2 score
	get_fore_pal(MSCORE_P);				// palette #2 score
...
void draw_agress_obj(void) 
{
	...
	*/
	...
	/* calc and set new width & height */
	pw=(DrawObj.dp_w<<8)/DrawObj.dp_scx;
	ph=(DrawObj.dp_h<<8)/DrawObj.dp_scy;
	
	/* adjust xy position */
	DrawObj.dp_x-=abs(pw-DrawObj.dp_w);
	DrawObj.dp_y-=abs(ph-DrawObj.dp_h);
	
	/* set xlu factors */
	DrawObj.dp_flags|=M_XLU_ON|M_RED_TINT;
	DrawObj.dp_flags|=HALF_XLU;
	
	gutil_draw_object();
	
	/* restore parameters */
	DrawObj.dp_flags=tflags;
	DrawObj.dp_scx=tscx;
	DrawObj.dp_scy=tscy;
	DrawObj.dp_x=tx;
	DrawObj.dp_y=ty;
		...
		if ( objpos.u.ypos>-250 && objpos.u.ypos<330 )
		{
			...
			else
			{
				...
				if (!(oflags2 & M_INVISO))
				{
					...
					/* Setup Draw Packet */				
					DrawObj.dp_palptr=&pal_buffer[(object_ptr->opal*2)*(256/sizeof(u64))];
					...
					gutil_draw_object();			// draw normal object
				}
			}
		}
	}
...
void shadow_code(OBJECT *obj,short sadj)
{
	...
	if (obj->oflags2 & M_SHADOW)
	{
		...
		DrawObj.dp_palptr=&pal_buffer[(shadow_clt*2)*(256/sizeof(u64))];
...
void floor_code(void)
{
	...
	/* Setup Draw Packet */				
	DrawObj.dp_palptr=&pal_buffer[(skew_constpal*2)*(256/sizeof(u64))];
