...
void sk_finish_him(void);
void motaro_finish_him(void);
...
{
	init_special_act(ACT_MOT_TELE,current_proc->pa8);

#if SHAO_PLAY==1
	if (am_i_joy()==SYSTEM_CARRY_SET)
		update_tsl(l_decoy,current_proc->pa8);
#endif
...
void sk_laugh(void)
{
	init_special_act(ACT_TAUNT,current_proc->pa8);
	lights_on_hit();
...
void sk_hammer(void)
{
	init_special_act(ACT_SK_HAMMER,current_proc->pa8);
	update_tsl(l_sz_decoy,current_proc->pa8);
...
void sk_kick(void)
{
	init_special_act(ACT_HIKICK,current_proc->pa8);
	group_sound(0);
...
void sk_punch(void)
{
	init_special_act(ACT_HIPUNCH,current_proc->pa8);
	group_sound(0);
...
void sk_charge(void)
{
	...
	init_special_act(ACT_SK_CHARGE,current_proc->pa8);
	...
	current_proc->pdata.p_action=ACT_SK_CHARGE_SD;
...
void sk_air_charge(void)
{
	...
	//init_special(obj);
	init_special_act(ACT_SK_AIRCHARGE,obj);
	...
	current_proc->pdata.p_action=ACT_SK_AIRCHAGE_SD;
...
void sk_taunt(void)
{
	...
	if (am_i_joy()==SYSTEM_CARRY_SET) 
	{
		current_proc->pdata.p_action=ACT_TAUNT;
		init_special_act(ACT_TAUNT,current_proc->pa8);
		lights_on_hit();
	}
#endif
...
void motaro_sweep(void)
{
	...
	current_proc->pdata.p_action=ACT_SWEEP;
...
void motaro_hop(void)
{
	...
	/* take off!! */
	mhop7(-SCX(0xa0000),-SCY(0xb0000),-1);
}
...
{
	current_proc->pdata.p_action=ACT_HIKICK;
...
*******************************************************************************/
void motaro_grab_punch_human(void)
{
	init_special(current_proc->pa8);

	if (b_get_x_dist<SCX(0xa0) && (current_proc->pdata.p_otherguy)->ochar!=FT_MOTARO)
...
void motaro_grab_punch_now(void)
{
	...
	current_proc->pdata.p_action=ACT_NOOGY;
...
{
	current_proc->pdata.p_action=ACT_HIPUNCH;
...
void skc_sz_zap(void) 
{
	if (randper(870)==SYSTEM_CARRY_CLR)
	{
		b_return_to_beware;
	}
...
JUMPTBL mot_counter_propell[]=
{
	...
	mc_propell_ls,	//c_mileena_roll	// 21a
	mc_propell_ls,	//c_mileena_roll	// 21b
	mc_propell_ls,	//c_mileena_roll	// 21c
	mc_propell_ls,	//c_mileena_roll	// 21d
	mc_propell_ls,	//c_mileena_roll	// 21e
	mc_propell_ls,	//c_mileena_roll	// 21f
	mc_propell_ls,	//c_mileena_roll	// 220
	mc_propell_ls,	//c_mileena_roll	// 221
	mc_propell_ls,	//c_mileena_roll	// 222
	mc_propell_ls,	//c_mileena_roll	// 223
};
...
void motaro_upcutted(void)
{
	...
	if (am_i_joy()==SYSTEM_CARRY_SET)
		process_sleep(0x10);
	else d_stance_pause(randu_minimum(0x10,0x10));
	b_reaction_exit();
}
...
extern void rd_cacell_victum(void);

void sk_finish_him(void)
{
	WORD ta10;
	
	d_stance_pause(0x20);				// dont look 2 much in a hurry
	
	if ((f_no_blood==0) && (randper(150)==SYSTEM_CARRY_SET))
	{
		ta10=0x40*4;
		d_stalk_a11(&ta10,SCX(0xb0));		// stalk till close
		sk_pound();
	}
	else
	{
		d_stance_pause(0x18);				// dont look 2 much in a hurry
		f_thatsall=1;
		takeover_him(rd_cacell_victum);
		d_stance_pause(0x8);				// dont look 2 much in a hurry
		f_death=-1;
		process_sleep(1);
		f_death=0;
...
void mot_fatal_head(void) 
{
	...
	me_in_back();
	stop_him(current_proc->pdata.p_otherguy);
	animate_a9(0x0003,7);			// reach out
	
	takeover_him(r_lose_head);
	process_sleep(40);
	
	/* move victory body and head to correct location */
	ohead=(OBJECT *)(current_proc->pdata.p_otherproc)->pdata.p_store2;
	
	/* victory frame 1 */
	do_next_a9_frame(obj);
	ohead->oxpos.u.intpos=obj->oxpos.u.intpos;
	ohead->oypos.u.intpos=obj->oypos.u.intpos;
	adjust_xy_a5(ohead,-35,24);
	ohead->ozval=obj->ozval+1;
	process_sleep(4);
...
void motaro_finish_him(void) 
{
	...
	if ((f_no_blood==0) && (randper(125)==SYSTEM_CARRY_SET))
	{
		ta10=0x40*4;
		d_stalk_a11(&ta10,SCX(0x50));		// stalk till close
		mot_fatal_head();
	}
	else
	{
