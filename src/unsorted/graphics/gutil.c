...
//////////////////////////////////////////////////////////////////////////////////////////
void gutil_clear_screen(unsigned short color ) 
{
	gDPPipeSync( Glp++ );
...
//	gDPSetRenderMode( Glp++, G_RM_OPA_SURF, G_RM_OPA_SURF2 );
	...
	gDPFillRectangle( Glp++, 0, 0, BUFFER_WIDTH-1,BUFFER_HEIGHT-1 );
	gDPPipeSync( Glp++ );
...
void gutil_draw_xlu_box(short x,short y,WORD w,WORD h,WORD color,WORD xlu) 
{
	...
	gDPFillRectangle(Glp++, x+xc, y+yc, (xc+x+w-1), (yc+y+h-1));
	gDPPipeSync( Glp++ );
	gDPSetRenderMode_default( Glp++ );
