...
void inc_his_p_hit(void)
{
	...
	if (p_agressor_active[player]==AGRESS_NORMAL)
		p_agressor[player]+=AGRESSOR_GROW*2;
...
void inc_p_block(void ) 
{
	...
	if (p_agressor_active[player]==AGRESS_NORMAL)
		p_agressor[player]+=(AGRESSOR_GROW*3);
	...
	while (TRUE)
	{
		...
		do
		{
			...
			*(pal+20)=*(pal+21)=*(pal+22)=*(pal+23)=*(pal+24)=*(pal+25)=*(pal+26)=*(pal+27)=*(pal+3);
			...
			/* setup xfer */
			//-SONY_IGNORE- palset_xfer((FORE_CLUT_START_X+((palid%CLUT_FACTOR)<<CLUT_SHIFT_SIZE)),(FORE_CLUT_START_Y+(palid/CLUT_FACTOR)),pal);
...
OBJECT *get_multi_obj(ADDRESS *framelist,void *mram,BYTE *dict,void *heapptr,WORD getpal)
{
	...
	if (getpal & PFLAG_FIGHTER)
	{
		...
		{
			p2_obj=obj;
//			p2_obj->oflags|=M_PLAYER_BUF2;
		}
		else 
		{
			p1_obj=obj;
//			p1_obj->oflags|=M_PLAYER_BUF1;
		}
	}
