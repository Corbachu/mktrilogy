...
#if 1		//-DHS- AGRESSOR STUFF 
	player=(obj==p1_obj)?0:1;
	
	if (p_agressor_active[player]!=0)			// set a trail for player to be special
		create_fx(FX_TRAIL)->pdata.p_store8=2;
#endif
...
WORD q_noob_ego(void)
{
	...
	if (process_exist(p1p2_pick(current_proc->pa8,PID_P1_EGO,PID_P2_EGO),0xffff)!=NULL)
		q_no;
	else q_yes;
}
...
{
	if (get_tsl(l_hp,current_proc->pa8)<0x50 || is_he_joy()==SYSTEM_CARRY_CLR || get_his_action(current_proc)==ACT_SHOCK_SD)
		q_no;
	else q_yes;
}
...
{
	if ((curback==BKGD_HADES_MOD ||curback==BKGD_PIT_MOD || curback==BKGD_BELL_MOD || curback==BKGD_SUBWAY_MOD) && f_no_blood==0)
	{
...
{
	if ((curback==BKGD_LAIR_MOD ||curback==BKGD_TOMB_MOD || curback==BKGD_DEADPOOL_MOD) && f_no_blood==0)
	{
		if ( (current_proc->pa8)->ochar==FT_ROBO1 || (current_proc->pa8)->ochar==FT_ROBO2 || (current_proc->pa8)->ochar==FT_SMOKE)
			return(q_fatal_dist(4,SCX(0x50)-8));
		else return(q_close_fatal());
