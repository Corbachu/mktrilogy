...
GEXPLODE ge_2on2[]=
{
	{2,SCX(0),SCY(0x20),SCY(0x4000),ARM_B01,a_arm2,(SCX(0x200)&0xfffe)+1,SCX(0x0),SCY(0xb00),SCY(0x400),3,6},
	{2,SCX(0),SCY(0x20),SCY(0x4000),LEG01,a_leg,(SCX(0x200)&0xfffe)+1,SCX(0x0),SCY(0xb00),SCY(0x400),3,6},
	{7,SCX(0),SCY(0x40),SCY(0x4000),BONE01,a_bone,(SCX(0x400)&0xfffe)+1,SCX(0x0),SCY(0xa00),SCY(0x400),3,6},
...
GEXPLODE  ge_laser[]=
{
 	...
 	{9,SCX(0x0),SCY(0x40),SCY(0x4000),BONE01,a_bone,(SCX(0x401)&0xfffe)+1,SCX(0x00),SCY(0xa00),SCY(0x400),3,3},
...
void fx_inviso_poof(void)
{
	...
	current_proc->a10=(ADDRESS)current_proc->pa8;
	//-SONY-get_char_ani(ANIM_TABLE2,ANIM_GROUND_EXP);
	current_proc->pa9=a_ground_explode;

	//-SONY-ptemp=gmo_proc((current_proc->pa8)->oheap);
...
void get_explode_obj(void)
{
	...
	//-SONY-ptemp=gmo_proc(&explode_fx_anims);
	ptemp=gmo_proc(explode_dict,0);
	obj=current_proc->pa8;
	//-SONY-obj->oheap=&explode_fx_anims;
	obj->oheap=0;
	(PROCESS *)obj->oplink=ptemp;
...
void trail_proc(void)
{
	...
	OBJECT *oa11;				// a11
	OBJECT *oa7;				// a7
	short index=current_proc->pdata.p_store8;
	...
	do
	{
		...
		get_multi_obj((ADDRESS *)current_proc->a5,&(ptemp->pdata),oa11->odict,oa11->oheap,PFLAG_PALLOAD);
		...
		else
		{
			...
			{
				/* agressor active */
				(current_proc->pa8)->oflags|=M_RED_TINT;
			}	
		}	
...
OBJECT *get_a_limb(WORD pa0)
{
	...
	//-SONY-animframe=get_char_ani(ANIM_TABLE2,limb_table[pa0]);
	//-SONY-animframe=(void*)COMPUTE_ADDR((current_proc->pa8)->oheap,GET_LONG(animframe));
	current_proc->pa9=animframe=limb_table[pa0];
	animframe=(void*)COMPUTE_ADDR(0,GET_LONG(animframe));
	//-SONY-gso_dmawnz(obj,(ADDRESS)animframe,(current_proc->pa8)->oheap,0);
	//-SONY-alloc_cache((OIMGTBL *)animframe,(current_proc->pa8)->oheap,obj);
	gso_dmawnz(obj,(ADDRESS)animframe,blood_dict,0,0);
	alloc_cache((OIMGTBL *)animframe,0,obj);
...
void get_ego_and_vel(void)
{
	...
	x=(long)((float) x * circle[index %23][0]);
	...
	y=(long)((float)y * circle[index %23][1]);
