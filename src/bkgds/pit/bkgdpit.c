...
long long wall_decomp[60*1024/sizeof(long long)];

/************************************************************
 * 			P I T / T H R O N E  S T U F F 					*
 ************************************************************/
extern void vcache_uncompress_lock(BYTE *dest,WORD ventry);
...
void build_pit_saw(void)
{
	...
	do
	{
		gso_dmawnz(obj,(ADDRESS)SAW2,pit_dict,0,0);
		alloc_cache((OIMGTBL *)SAW2,0,obj);
...
void pit_scroll_proc(void)
{
	...
	gso_dmawnz_ns(obj,(ADDRESS)&THRONEWALL,pit_dict,0,0);			// left side slab
	alloc_cache((OIMGTBL *)&THRONEWALL,0,obj);
	set_xy_coordinates(obj,0,SCY(0xfe)+21);
	oa9=obj;
	insert_object(obj,&baklst4);

	gso_dmawnz_ns(obj,(ADDRESS)&THRONEWALL,pit_dict,0,0);			// right side slab
	alloc_cache((OIMGTBL *)&THRONEWALL,0,obj);
	flip_single(obj);
	...
	/* decomp wall in temp area */
	vcache_uncompress_lock((BYTE *)wall_decomp,obj->ovcache);
